...
9. Open **Persistent Storage** and confirm that MongoDB has a volume mounted
   at `/data/db`. Do not delete this volume when redeploying the backend.
10. Copy the database's **Internal URL** exactly as Coolify displays it.
...
- Do not add a trailing slash to `CLIENT_ORIGINS`.
- Use the MongoDB **Internal URL**, not a public IP or public URL.
- `EMAIL_PASSWORD` must be a Gmail app password, not the normal account
  password.
...
| Browser reports CORS errors | `CLIENT_ORIGINS` must exactly equal `https://flixdrive.minimanx.dev` |
