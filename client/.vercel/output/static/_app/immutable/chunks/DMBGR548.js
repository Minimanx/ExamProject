var e = Object.defineProperty,
    t = (t, n) => {
        let r = {};
        for (var i in t) e(r, i, { get: t[i], enumerable: !0 });
        return (n || e(r, Symbol.toStringTag, { value: `Module` }), r);
    },
    n = Array.isArray,
    r = Array.prototype.indexOf,
    i = Array.prototype.includes,
    a = Array.from,
    o = Object.defineProperty,
    s = Object.getOwnPropertyDescriptor,
    c = Object.getOwnPropertyDescriptors,
    l = Object.prototype,
    u = Array.prototype,
    d = Object.getPrototypeOf,
    f = Object.isExtensible;
function p(e) {
    return typeof e == `function`;
}
var m = () => {};
function h(e) {
    return e();
}
function g(e) {
    for (var t = 0; t < e.length; t++) e[t]();
}
function _() {
    var e, t;
    return {
        promise: new Promise((n, r) => {
            ((e = n), (t = r));
        }),
        resolve: e,
        reject: t,
    };
}
var v = 1024,
    y = 2048,
    b = 4096,
    x = 8192,
    S = 16384,
    ee = 32768,
    te = 1 << 25,
    ne = 65536,
    re = 1 << 19,
    ie = 1 << 20,
    ae = 1 << 25,
    oe = 65536,
    se = 1 << 21,
    ce = 1 << 22,
    le = 1 << 23,
    ue = Symbol(`$state`),
    de = Symbol(`legacy props`),
    fe = Symbol(``),
    pe = Symbol(`attributes`),
    me = Symbol(`class`),
    he = Symbol(`style`),
    ge = Symbol(`text`),
    _e = Symbol(`form reset`),
    ve = new (class extends Error {
        name = `StaleReactionError`;
        message = "The reaction that called `getAbortSignal()` was re-run or destroyed";
    })(),
    ye = !!globalThis.document?.contentType && globalThis.document.contentType.includes(`xml`);
function be(e) {
    throw Error(`https://svelte.dev/e/experimental_async_required`);
}
function xe(e) {
    throw Error(`https://svelte.dev/e/lifecycle_outside_component`);
}
function Se() {
    throw Error(`https://svelte.dev/e/missing_context`);
}
function Ce() {
    throw Error(`https://svelte.dev/e/async_derived_orphan`);
}
function we(e, t, n) {
    throw Error(`https://svelte.dev/e/each_key_duplicate`);
}
function Te(e) {
    throw Error(`https://svelte.dev/e/effect_in_teardown`);
}
function Ee() {
    throw Error(`https://svelte.dev/e/effect_in_unowned_derived`);
}
function De(e) {
    throw Error(`https://svelte.dev/e/effect_orphan`);
}
function Oe() {
    throw Error(`https://svelte.dev/e/effect_update_depth_exceeded`);
}
function ke() {
    throw Error(`https://svelte.dev/e/fork_discarded`);
}
function Ae() {
    throw Error(`https://svelte.dev/e/fork_timing`);
}
function je() {
    throw Error(`https://svelte.dev/e/get_abort_signal_outside_reaction`);
}
function Me() {
    throw Error(`https://svelte.dev/e/hydration_failed`);
}
function Ne(e) {
    throw Error(`https://svelte.dev/e/lifecycle_legacy_only`);
}
function Pe(e) {
    throw Error(`https://svelte.dev/e/props_invalid_value`);
}
function Fe() {
    throw Error(`https://svelte.dev/e/state_descriptors_fixed`);
}
function Ie() {
    throw Error(`https://svelte.dev/e/state_prototype_fixed`);
}
function Le() {
    throw Error(`https://svelte.dev/e/state_unsafe_mutation`);
}
function Re() {
    throw Error(`https://svelte.dev/e/svelte_boundary_reset_onerror`);
}
var ze = {},
    C = Symbol(`uninitialized`),
    Be = `http://www.w3.org/1999/xhtml`,
    Ve = `http://www.w3.org/2000/svg`,
    He = `http://www.w3.org/1998/Math/MathML`;
function Ue() {
    console.warn(`https://svelte.dev/e/derived_inert`);
}
function We(e) {
    console.warn(`https://svelte.dev/e/hydratable_missing_but_expected`);
}
function Ge(e) {
    console.warn(`https://svelte.dev/e/hydration_mismatch`);
}
function Ke() {
    console.warn(`https://svelte.dev/e/svelte_boundary_reset_noop`);
}
var w = !1;
function T(e) {
    w = e;
}
var E;
function D(e) {
    if (e === null) throw (Ge(), ze);
    return (E = e);
}
function qe() {
    return D(L(E));
}
function Je(e) {
    if (w) {
        if (L(E) !== null) throw (Ge(), ze);
        E = e;
    }
}
function Ye(e = 1) {
    if (w) {
        for (var t = e, n = E; t--;) n = L(n);
        E = n;
    }
}
function Xe(e = !0) {
    for (var t = 0, n = E; ;) {
        if (n.nodeType === 8) {
            var r = n.data;
            if (r === `]`) {
                if (t === 0) return n;
                --t;
            } else
                (r === `[` || r === `[!` || (r[0] === `[` && !isNaN(Number(r.slice(1))))) &&
                    (t += 1);
        }
        var i = L(n);
        (e && n.remove(), (n = i));
    }
}
function Ze(e) {
    if (!e || e.nodeType !== 8) throw (Ge(), ze);
    return e.data;
}
function Qe(e) {
    return e === this.v;
}
function $e(e, t) {
    return e == e ? e !== t || (typeof e == `object` && !!e) || typeof e == `function` : t == t;
}
function et(e) {
    return !$e(e, this.v);
}
var tt = !1;
function nt() {
    tt = !0;
}
var O = null;
function rt(e) {
    O = e;
}
function it() {
    let e = {};
    return [() => (st(e) || Se(), at(e)), (t) => ot(e, t)];
}
function at(e) {
    return ft(`getContext`).get(e);
}
function ot(e, t) {
    return (ft(`setContext`).set(e, t), t);
}
function st(e) {
    return ft(`hasContext`).has(e);
}
function ct() {
    return ft(`getAllContexts`);
}
function lt(e, t = !1, n) {
    O = {
        p: O,
        i: !1,
        c: null,
        e: null,
        s: e,
        x: null,
        r: W,
        l: tt && !t ? { s: null, u: null, $: [] } : null,
    };
}
function ut(e) {
    var t = O,
        n = t.e;
    if (n !== null) {
        t.e = null;
        for (var r of n) Qn(r);
    }
    return (e !== void 0 && (t.x = e), (t.i = !0), (O = t.p), e ?? {});
}
function dt() {
    return !tt || (O !== null && O.l === null);
}
function ft(e) {
    return (O === null && xe(e), (O.c ??= new Map(pt(O) || void 0)));
}
function pt(e) {
    let t = e.p;
    for (; t !== null;) {
        let e = t.c;
        if (e !== null) return e;
        t = t.p;
    }
    return null;
}
var mt = [];
function ht() {
    var e = mt;
    ((mt = []), g(e));
}
function k(e) {
    if (mt.length === 0 && !cn) {
        var t = mt;
        queueMicrotask(() => {
            t === mt && ht();
        });
    }
    mt.push(e);
}
function gt() {
    for (; mt.length > 0;) ht();
}
function _t(e) {
    var t = W;
    if (t === null) return ((V.f |= le), e);
    if (!(t.f & 32768) && !(t.f & 4)) throw e;
    vt(e, t);
}
function vt(e, t) {
    if (!(t !== null && t.f & 16384)) {
        for (; t !== null;) {
            if (t.f & 128) {
                if (!(t.f & 32768)) throw e;
                try {
                    t.b.error(e);
                    return;
                } catch (t) {
                    e = t;
                }
            }
            t = t.parent;
        }
        throw e;
    }
}
var yt = ~(y | b | v);
function A(e, t) {
    e.f = (e.f & yt) | t;
}
function bt(e) {
    e.f & 512 || e.deps === null ? A(e, v) : A(e, b);
}
function xt(e) {
    if (e !== null) for (let t of e) !(t.f & 2) || !(t.f & 65536) || ((t.f ^= oe), xt(t.deps));
}
function St(e, t, n) {
    (e.f & 2048 ? t.add(e) : e.f & 4096 && n.add(e), xt(e.deps), A(e, v));
}
function Ct(e, t, n) {
    if (e == null) return (t(void 0), n && n(void 0), m);
    let r = Z(() => e.subscribe(t, n));
    return r.unsubscribe ? () => r.unsubscribe() : r;
}
var wt = [];
function Tt(e, t = m) {
    let n = null,
        r = new Set();
    function i(t) {
        if ($e(e, t) && ((e = t), n)) {
            let t = !wt.length;
            for (let t of r) (t[1](), wt.push(t, e));
            if (t) {
                for (let e = 0; e < wt.length; e += 2) wt[e][0](wt[e + 1]);
                wt.length = 0;
            }
        }
    }
    function a(t) {
        i(t(e));
    }
    function o(o, s = m) {
        let c = [o, s];
        return (
            r.add(c),
            r.size === 1 && (n = t(i, a) || m),
            o(e),
            () => {
                (r.delete(c), r.size === 0 && n && (n(), (n = null)));
            }
        );
    }
    return { set: i, update: a, subscribe: o };
}
function Et(e) {
    let t;
    return (Ct(e, (e) => (t = e))(), t);
}
var Dt = !1,
    Ot = !1,
    kt = Symbol(`unmounted`);
function At(e, t, n) {
    let r = (n[t] ??= { store: null, source: kn(void 0), unsubscribe: m });
    if (r.store !== e && !(kt in n)) {
        if ((r.unsubscribe(), (r.store = e ?? null), e == null))
            ((r.source.v = void 0), (r.unsubscribe = m));
        else {
            var i = !0;
            ((r.unsubscribe = Ct(e, (e) => {
                i ? (r.source.v = e) : P(r.source, e);
            })),
                (i = !1));
        }
    }
    return e && kt in n ? Et(e) : X(r.source);
}
function jt(e, t) {
    return (Nt(e, t), t);
}
function Mt() {
    let e = {};
    function t() {
        Xn(() => {
            for (var t in e) e[t].unsubscribe();
            o(e, kt, { enumerable: !1, value: !0 });
        });
    }
    return [e, t];
}
function Nt(e, t) {
    Dt = !0;
    try {
        e.set(t);
    } finally {
        Dt = !1;
    }
}
function Pt(e, t, n) {
    return (Nt(e, n), t);
}
function Ft(e) {
    var t = Ot;
    try {
        return ((Ot = !1), [e(), Ot]);
    } finally {
        Ot = t;
    }
}
var It = !1;
function Lt() {
    It ||
        ((It = !0),
        document.addEventListener(
            `reset`,
            (e) => {
                Promise.resolve().then(() => {
                    if (!e.defaultPrevented) for (let t of e.target.elements) t[_e]?.();
                });
            },
            { capture: !0 }
        ));
}
function Rt(e) {
    var t = V,
        n = W;
    (U(null), G(null));
    try {
        return e();
    } finally {
        (U(t), G(n));
    }
}
function zt(e, t, n, r = n) {
    e.addEventListener(t, () => Rt(n));
    let i = e[_e];
    ((e[_e] = i
        ? () => {
              (i(), r(!0));
          }
        : () => r(!0)),
        Lt());
}
function Bt(e) {
    let t = 0,
        n = Dn(0),
        r;
    return () => {
        Yn() &&
            (X(n),
            ar(
                () => (
                    t === 0 && (r = Z(() => e(() => Mn(n)))),
                    (t += 1),
                    () => {
                        k(() => {
                            (--t, t === 0 && (r?.(), (r = void 0), Mn(n)));
                        });
                    }
                )
            ));
    };
}
var Vt = ne | re;
function Ht(e, t, n, r) {
    new Ut(e, t, n, r);
}
var Ut = class {
    parent;
    is_pending = !1;
    transform_error;
    #e;
    #t = w ? E : null;
    #n;
    #r;
    #i;
    #a = null;
    #o = null;
    #s = null;
    #c = null;
    #l = 0;
    #u = 0;
    #d = !1;
    #f = new Set();
    #p = new Set();
    #m = null;
    #h = Bt(
        () => (
            (this.#m = Dn(this.#l)),
            () => {
                this.#m = null;
            }
        )
    );
    constructor(e, t, n, r) {
        ((this.#e = e),
            (this.#n = t),
            (this.#r = (e) => {
                var t = W;
                ((t.b = this), (t.f |= 128), n(e));
            }),
            (this.parent = W.b),
            (this.transform_error = r ?? this.parent?.transform_error ?? ((e) => e)),
            (this.#i = sr(() => {
                if (w) {
                    let e = this.#t;
                    qe();
                    let t = e.data === `[!`;
                    if (e.data.startsWith(`[?`)) {
                        let t = JSON.parse(e.data.slice(2));
                        this.#_(t);
                    } else t ? this.#y() : this.#g();
                } else this.#b();
            }, Vt)),
            w && (this.#e = E));
    }
    #g() {
        try {
            this.#a = z(() => this.#r(this.#e));
        } catch (e) {
            this.error(e);
        }
    }
    #_(e) {
        let t = this.#n.failed,
            { reset: n, invoke_onerror: r } = this.#v(e);
        (k(r),
            t &&
                (this.#s = z(() => {
                    t(
                        this.#e,
                        () => e,
                        () => n
                    );
                })));
    }
    #v(e) {
        var t = !1,
            n = !1;
        let r = () => {
            if (t) {
                Ke();
                return;
            }
            ((t = !0),
                n && Re(),
                this.#s !== null &&
                    pr(this.#s, () => {
                        this.#s = null;
                    }),
                this.#S(() => {
                    this.#b();
                }));
        };
        return {
            reset: r,
            invoke_onerror: () => {
                try {
                    ((n = !0), this.#n.onerror?.(e, r), (n = !1));
                } catch (e) {
                    vt(e, this.#i && this.#i.parent);
                }
            },
        };
    }
    #y() {
        let e = this.#n.pending;
        e &&
            ((this.is_pending = !0),
            (this.#o = z(() => e(this.#e))),
            k(() => {
                var e = (this.#c = document.createDocumentFragment()),
                    t = F();
                (e.append(t),
                    (this.#a = this.#S(() => z(() => this.#r(t)))),
                    this.#u === 0 &&
                        (this.#e.before(e),
                        (this.#c = null),
                        pr(this.#o, () => {
                            this.#o = null;
                        }),
                        this.#x(j)));
            }));
    }
    #b() {
        try {
            if (
                ((this.is_pending = this.has_pending_snippet()),
                (this.#u = 0),
                (this.#l = 0),
                (this.#a = z(() => {
                    this.#r(this.#e);
                })),
                this.#u > 0)
            ) {
                var e = (this.#c = document.createDocumentFragment());
                _r(this.#a, e);
                let t = this.#n.pending;
                this.#o = z(() => t(this.#e));
            } else this.#x(j);
        } catch (e) {
            this.error(e);
        }
    }
    #x(e) {
        ((this.is_pending = !1), e.transfer_effects(this.#f, this.#p));
    }
    defer_effect(e) {
        St(e, this.#f, this.#p);
    }
    is_rendered() {
        return !this.is_pending && (!this.parent || this.parent.is_rendered());
    }
    has_pending_snippet() {
        return !!this.#n.pending;
    }
    #S(e) {
        var t = W,
            n = V,
            r = O;
        (G(this.#i), U(this.#i), rt(this.#i.ctx));
        try {
            return (mn.ensure(), e());
        } catch (e) {
            return (_t(e), null);
        } finally {
            (G(t), U(n), rt(r));
        }
    }
    #C(e, t) {
        if (!this.has_pending_snippet()) {
            this.parent && this.parent.#C(e, t);
            return;
        }
        ((this.#u += e),
            this.#u === 0 &&
                (this.#x(t),
                this.#o &&
                    pr(this.#o, () => {
                        this.#o = null;
                    }),
                (this.#c &&= (this.#e.before(this.#c), null))));
    }
    update_pending_count(e, t) {
        (this.#C(e, t),
            (this.#l += e),
            !(!this.#m || this.#d) &&
                ((this.#d = !0),
                k(() => {
                    ((this.#d = !1), this.#m && An(this.#m, this.#l));
                })));
    }
    get_effect_pending() {
        return (this.#h(), X(this.#m));
    }
    error(e) {
        if (!this.#n.onerror && !this.#n.failed) throw e;
        j?.is_fork
            ? (this.#a && j.skip_effect(this.#a),
              this.#o && j.skip_effect(this.#o),
              this.#s && j.skip_effect(this.#s),
              j.oncommit(() => {
                  this.#w(e);
              }))
            : this.#w(e);
    }
    #w(e) {
        ((this.#a &&= (B(this.#a), null)),
            (this.#o &&= (B(this.#o), null)),
            (this.#s &&= (B(this.#s), null)),
            w && (D(this.#t), Ye(), D(Xe())));
        let t = this.#n.failed,
            n = (e) => {
                let { reset: n, invoke_onerror: r } = this.#v(e);
                (r(),
                    t &&
                        (this.#s = this.#S(() => {
                            try {
                                return z(() => {
                                    var r = W;
                                    ((r.b = this),
                                        (r.f |= 128),
                                        t(
                                            this.#e,
                                            () => e,
                                            () => n
                                        ));
                                });
                            } catch (e) {
                                return (vt(e, this.#i.parent), null);
                            }
                        })));
            };
        k(() => {
            var t;
            try {
                t = this.transform_error(e);
            } catch (e) {
                vt(e, this.#i && this.#i.parent);
                return;
            }
            typeof t == `object` && t && typeof t.then == `function`
                ? t.then(n, (e) => vt(e, this.#i && this.#i.parent))
                : n(t);
        });
    }
};
function Wt(e, t, n, r) {
    let i = dt() ? Jt : Qt;
    var a = e.filter((e) => !e.settled),
        o = t.map(i);
    if (n.length === 0 && a.length === 0) {
        r(o);
        return;
    }
    var s = W,
        c = Gt(),
        l =
            a.length === 1
                ? a[0].promise
                : a.length > 1
                  ? Promise.all(a.map((e) => e.promise))
                  : null;
    function u(e) {
        if (!(s.f & 16384)) {
            c();
            try {
                r([...o, ...e]);
            } catch (e) {
                vt(e, s);
            }
            Kt();
        }
    }
    var d = qt();
    if (n.length === 0) {
        l.then(() => u([])).finally(d);
        return;
    }
    function f() {
        Promise.all(n.map((e) => Xt(e)))
            .then(u)
            .catch((e) => vt(e, s))
            .finally(d);
    }
    l
        ? l.then(() => {
              (c(), f(), Kt());
          })
        : f();
}
function Gt() {
    var e = W,
        t = V,
        n = O,
        r = j;
    return function (i = !0) {
        (G(e), U(t), rt(n), i && !(e.f & 16384) && (r?.activate(), r?.apply()));
    };
}
function Kt(e = !0) {
    (G(null), U(null), rt(null), e && j?.deactivate());
}
function qt() {
    var e = W,
        t = e.b,
        n = j,
        r = !!t?.is_rendered();
    return (
        t?.update_pending_count(1, n),
        n.increment(r, e),
        () => {
            (t?.update_pending_count(-1, n), n.decrement(r, e));
        }
    );
}
function Jt(e) {
    var t = 2 | y;
    return (
        W !== null && (W.f |= re),
        {
            ctx: O,
            deps: null,
            effects: null,
            equals: Qe,
            f: t,
            fn: e,
            reactions: null,
            rv: 0,
            v: C,
            wv: 0,
            parent: W,
            ac: null,
        }
    );
}
var Yt = Symbol(`obsolete`);
function Xt(e, t, n) {
    let r = W;
    r === null && Ce();
    var i = void 0,
        a = Dn(C),
        o = !V,
        s = new Set();
    return (
        ir(() => {
            var t = W,
                n = _();
            i = n.promise;
            try {
                Promise.resolve(e())
                    .then(n.resolve, (e) => {
                        e !== ve && n.reject(e);
                    })
                    .finally(Kt);
            } catch (e) {
                (n.reject(e), Kt());
            }
            var c = j;
            if (o) {
                if (t.f & 32768) var l = qt();
                if (r.b?.is_rendered()) c.async_deriveds.get(t)?.reject(Yt);
                else for (let e of s.values()) e.reject(Yt);
                (s.add(n), c.async_deriveds.set(t, n));
            }
            let u = (e, t = void 0) => {
                (l?.(),
                    s.delete(n),
                    t !== Yt &&
                        (c.activate(),
                        t ? ((a.f |= le), An(a, t)) : (a.f & 8388608 && (a.f ^= le), An(a, e)),
                        c.deactivate()));
            };
            n.promise.then(u, (e) => u(null, e || `unknown`));
        }),
        Xn(() => {
            for (let e of s) e.reject(Yt);
        }),
        new Promise((e) => {
            function t(n) {
                function r() {
                    n === i ? e(a) : t(i);
                }
                n.then(r, r);
            }
            t(i);
        })
    );
}
function Zt(e) {
    let t = Jt(e);
    return (Sr(t), t);
}
function Qt(e) {
    let t = Jt(e);
    return ((t.equals = et), t);
}
function $t(e) {
    var t = e.effects;
    if (t !== null) {
        e.effects = null;
        for (var n = 0; n < t.length; n += 1) B(t[n]);
    }
}
function en(e) {
    var t,
        n = W,
        r = e.parent;
    if (!br && r !== null && e.v !== C && r.f & 24576) return (Ue(), e.v);
    G(r);
    try {
        ((e.f &= ~oe), $t(e), (t = jr(e)));
    } finally {
        G(n);
    }
    return t;
}
function tn(e) {
    var t = en(e);
    if (
        !e.equals(t) &&
        ((e.wv = Or()),
        (!j?.is_fork || e.deps === null) &&
            (j === null ? (e.v = t) : (j.capture(e, t, !0), on?.capture(e, t, !0)),
            e.deps === null))
    ) {
        A(e, v);
        return;
    }
    br || (M === null ? bt(e) : (Yn() || j?.is_fork) && M.set(e, t));
}
function nn(e) {
    if (e.effects !== null)
        for (let t of e.effects)
            (t.teardown || t.ac) &&
                (t.teardown?.(),
                t.ac !== null &&
                    Rt(() => {
                        (t.ac.abort(ve), (t.ac = null));
                    }),
                t.fn !== null && (t.teardown = m),
                Nr(t, 0),
                lr(t));
}
function rn(e) {
    if (e.effects !== null) for (let t of e.effects) t.teardown && t.fn !== null && Pr(t);
}
var an = null,
    j = null,
    on = null,
    M = null,
    sn = null,
    cn = !1,
    ln = !1,
    un = null,
    dn = null,
    fn = 0,
    pn = 1,
    mn = class e {
        id = pn++;
        #e = !1;
        linked = !0;
        #t = null;
        #n = null;
        async_deriveds = new Map();
        current = new Map();
        previous = new Map();
        #r = new Set();
        #i = new Set();
        #a = 0;
        #o = new Map();
        #s = null;
        #c = [];
        #l = [];
        #u = new Set();
        #d = new Set();
        #f = new Map();
        #p = new Set();
        is_fork = !1;
        #m = !1;
        constructor() {
            (an === null ? (an = this) : ((an.#n = this), (this.#t = an)), (an = this));
        }
        #h() {
            if (this.is_fork) return !0;
            for (let n of this.#o.keys()) {
                for (var e = n, t = !1; e.parent !== null;) {
                    if (this.#f.has(e)) {
                        t = !0;
                        break;
                    }
                    e = e.parent;
                }
                if (!t) return !0;
            }
            return !1;
        }
        skip_effect(e) {
            (this.#f.has(e) || this.#f.set(e, { d: [], m: [] }), this.#p.delete(e));
        }
        unskip_effect(e, t = (e) => this.schedule(e)) {
            var n = this.#f.get(e);
            if (n) {
                this.#f.delete(e);
                for (var r of n.d) (A(r, y), t(r));
                for (r of n.m) (A(r, b), t(r));
            }
            this.#p.add(e);
        }
        #g() {
            ((this.#e = !0), fn++ > 1e3 && (this.#x(), gn()));
            for (let e of this.#u) (this.#d.delete(e), A(e, y), this.schedule(e));
            for (let e of this.#d) (A(e, b), this.schedule(e));
            let t = this.#c;
            ((this.#c = []), this.apply());
            var n = (un = []),
                r = [],
                i = (dn = []);
            for (let e of t)
                try {
                    this.#_(e, n, r);
                } catch (t) {
                    throw (xn(e), this.#h() || this.discard(), t);
                }
            if (((j = null), i.length > 0)) {
                var a = e.ensure();
                for (let e of i) a.schedule(e);
            }
            if (((un = null), (dn = null), this.#h())) {
                (this.#b(r), this.#b(n));
                for (let [e, t] of this.#f) bn(e, t);
                i.length > 0 && j.#g();
                return;
            }
            let o = this.#v();
            if (o) {
                (this.#b(r), this.#b(n), o.#y(this));
                return;
            }
            (this.#u.clear(), this.#d.clear());
            for (let e of this.#r) e(this);
            (this.#r.clear(), (on = this), _n(r), _n(n), (on = null), this.#s?.resolve());
            var s = j;
            if (
                (this.#a === 0 && (this.#c.length === 0 || s !== null) && this.#x(),
                this.#c.length > 0)
            ) {
                if (s !== null) {
                    let e = s;
                    e.#c.push(...this.#c.filter((t) => !e.#c.includes(t)));
                } else s = this;
            }
            s !== null && s.#g();
        }
        #_(e, t, n) {
            e.f ^= v;
            for (var r = e.first; r !== null;) {
                var i = r.f,
                    a = !!(i & 96);
                if (!((a && i & 1024) || i & 8192 || this.#f.has(r)) && r.fn !== null) {
                    a ? (r.f ^= v) : i & 4 ? t.push(r) : kr(r) && (i & 16 && this.#d.add(r), Pr(r));
                    var o = r.first;
                    if (o !== null) {
                        r = o;
                        continue;
                    }
                }
                for (; r !== null;) {
                    var s = r.next;
                    if (s !== null) {
                        r = s;
                        break;
                    }
                    r = r.parent;
                }
            }
        }
        #v() {
            for (var e = this.#t; e !== null;) {
                if (!e.is_fork) {
                    for (let [t, [, n]] of this.current) if (e.current.has(t) && !n) return e;
                }
                e = e.#t;
            }
            return null;
        }
        #y(e) {
            for (let [t, n] of e.current)
                (!this.previous.has(t) &&
                    e.previous.has(t) &&
                    this.previous.set(t, e.previous.get(t)),
                    this.current.set(t, n));
            for (let [t, n] of e.async_deriveds) {
                let e = this.async_deriveds.get(t);
                e && n.promise.then(e.resolve).catch(e.reject);
            }
            (e.async_deriveds.clear(), this.transfer_effects(e.#u, e.#d));
            let t = (e) => {
                var n = e.reactions;
                if (n !== null && !(e.f & 2 && !(e.f & 6144)))
                    for (let e of n) {
                        var r = e.f;
                        if (r & 2) t(e);
                        else {
                            var i = e;
                            r & 4194320 &&
                                !this.async_deriveds.has(i) &&
                                (this.#d.delete(i), A(i, y), this.schedule(i));
                        }
                    }
            };
            for (let e of this.current.keys()) t(e);
            (this.oncommit(() => e.discard()), e.#x(), (j = this), this.#g());
        }
        #b(e) {
            for (var t = 0; t < e.length; t += 1) St(e[t], this.#u, this.#d);
        }
        capture(e, t, n = !1) {
            (e.v !== C && !this.previous.has(e) && this.previous.set(e, e.v),
                e.f & 8388608 || (this.current.set(e, [t, n]), M?.set(e, t)),
                this.is_fork || (e.v = t));
        }
        activate() {
            j = this;
        }
        deactivate() {
            ((j = null), (M = null));
        }
        flush() {
            try {
                ((ln = !0), (j = this), this.#g());
            } finally {
                ((fn = 0),
                    (sn = null),
                    (un = null),
                    (dn = null),
                    (ln = !1),
                    (j = null),
                    (M = null),
                    wn.clear());
            }
        }
        discard() {
            for (let e of this.#i) e(this);
            this.#i.clear();
            for (let e of this.async_deriveds.values()) e.reject(Yt);
            (this.#x(), this.#s?.resolve());
        }
        register_created_effect(e) {
            this.#l.push(e);
        }
        increment(e, t) {
            if (((this.#a += 1), e)) {
                let e = this.#o.get(t) ?? 0;
                this.#o.set(t, e + 1);
            }
        }
        decrement(e, t) {
            if ((--this.#a, e)) {
                let e = this.#o.get(t) ?? 0;
                e === 1 ? this.#o.delete(t) : this.#o.set(t, e - 1);
            }
            this.#m ||
                ((this.#m = !0),
                k(() => {
                    ((this.#m = !1), this.linked && this.flush());
                }));
        }
        transfer_effects(e, t) {
            for (let t of e) this.#u.add(t);
            for (let e of t) this.#d.add(e);
            (e.clear(), t.clear());
        }
        oncommit(e) {
            this.#r.add(e);
        }
        ondiscard(e) {
            this.#i.add(e);
        }
        settled() {
            return (this.#s ??= _()).promise;
        }
        static ensure() {
            if (j === null) {
                let t = (j = new e());
                !ln &&
                    !cn &&
                    k(() => {
                        t.#e || t.flush();
                    });
            }
            return j;
        }
        apply() {
            M = null;
        }
        schedule(e) {
            if (((sn = e), e.b?.is_pending && e.f & 16777228 && !(e.f & 32768))) {
                e.b.defer_effect(e);
                return;
            }
            for (var t = e; t.parent !== null;) {
                t = t.parent;
                var n = t.f;
                if (un !== null && t === W && (V === null || !(V.f & 2)) && !Dt) return;
                if (n & 96) {
                    if (!(n & 1024)) return;
                    t.f ^= v;
                }
            }
            this.#c.push(t);
        }
        #x() {
            if (this.linked) {
                var e = this.#t,
                    t = this.#n;
                (e === null || (e.#n = t), t === null ? (an = e) : (t.#t = e), (this.linked = !1));
            }
        }
    };
function hn(e) {
    var t = cn;
    cn = !0;
    try {
        var n;
        for (e && (j !== null && !j.is_fork && j.flush(), (n = e())); ;) {
            if ((gt(), j === null)) return n;
            j.flush();
        }
    } finally {
        cn = t;
    }
}
function gn() {
    try {
        Oe();
    } catch (e) {
        vt(e, sn);
    }
}
var N = null;
function _n(e) {
    var t = e.length;
    if (t !== 0) {
        for (var n = 0; n < t;) {
            var r = e[n++];
            if (
                !(r.f & 24576) &&
                kr(r) &&
                ((N = new Set()),
                Pr(r),
                r.deps === null &&
                    r.first === null &&
                    r.nodes === null &&
                    r.teardown === null &&
                    r.ac === null &&
                    fr(r),
                N?.size > 0)
            ) {
                wn.clear();
                for (let e of N) {
                    if (e.f & 24576) continue;
                    let t = [e],
                        n = e.parent;
                    for (; n !== null;) (N.has(n) && (N.delete(n), t.push(n)), (n = n.parent));
                    for (let e = t.length - 1; e >= 0; e--) {
                        let n = t[e];
                        n.f & 24576 || Pr(n);
                    }
                }
                N.clear();
            }
        }
        N = null;
    }
}
function vn(e, t) {
    if (e.reactions !== null)
        for (let n of e.reactions) {
            let e = n.f;
            e & 2 ? vn(n, t) : e & 131072 && (A(n, y), t.add(n));
        }
}
function yn(e) {
    j.schedule(e);
}
function bn(e, t) {
    if (!(e.f & 32 && e.f & 1024)) {
        (e.f & 2048 ? t.d.push(e) : e.f & 4096 && t.m.push(e), A(e, v));
        for (var n = e.first; n !== null;) (bn(n, t), (n = n.next));
    }
}
function xn(e) {
    A(e, v);
    for (var t = e.first; t !== null;) (xn(t), (t = t.next));
}
function Sn(e) {
    (be(`fork`), j !== null && Ae());
    var t = mn.ensure();
    ((t.is_fork = !0), (M = new Map()));
    var n = !1,
        r = t.settled();
    return (
        hn(e),
        {
            commit: async () => {
                if (n) {
                    await r;
                    return;
                }
                (t.linked || ke(), (n = !0), (t.is_fork = !1));
                for (var [e, [i]] of t.current) ((e.v = i), (e.wv = Or()));
                (hn(() => {
                    var e = new Set();
                    for (var n of t.current.keys()) vn(n, e);
                    (Tn(e), jn());
                }),
                    t.flush(),
                    await r);
            },
            discard: () => {
                for (var e of t.current.keys()) e.wv = Or();
                !n && t.linked && t.discard();
            },
        }
    );
}
var Cn = new Set(),
    wn = new Map();
function Tn(e) {
    Cn = e;
}
var En = !1;
function Dn(e, t) {
    return { f: 0, v: e, reactions: null, equals: Qe, rv: 0, wv: 0 };
}
function On(e, t) {
    let n = Dn(e, t);
    return (Sr(n), n);
}
function kn(e, t = !1, n = !0) {
    let r = Dn(e);
    return (
        t || (r.equals = et),
        tt && n && O !== null && O.l !== null && (O.l.s ??= []).push(r),
        r
    );
}
function P(e, t, n = !1) {
    return (
        V !== null &&
            (!H || V.f & 131072) &&
            dt() &&
            V.f & 4325394 &&
            (K === null || !K.has(e)) &&
            Le(),
        An(e, n ? Pn(t) : t, dn)
    );
}
function An(e, t, n = null) {
    if (!e.equals(t)) {
        wn.set(e, br ? t : e.v);
        var r = mn.ensure();
        if ((r.capture(e, t), e.f & 2)) {
            let t = e;
            (e.f & 2048 && en(t), M === null && bt(t));
        }
        ((e.wv = Or()),
            Nn(e, y, n),
            dt() && W !== null && W.f & 1024 && !(W.f & 96) && (Y === null ? Cr([e]) : Y.push(e)),
            !r.is_fork && Cn.size > 0 && !En && jn());
    }
    return t;
}
function jn() {
    En = !1;
    for (let e of Cn) {
        e.f & 1024 && A(e, b);
        let t;
        try {
            t = kr(e);
        } catch {
            t = !0;
        }
        t && Pr(e);
    }
    Cn.clear();
}
function Mn(e) {
    P(e, e.v + 1);
}
function Nn(e, t, n) {
    var r = e.reactions;
    if (r !== null)
        for (var i = dt(), a = r.length, o = 0; o < a; o++) {
            var s = r[o],
                c = s.f;
            if (!(!i && s === W)) {
                var l = (c & y) === 0;
                if ((l && A(s, t), c & 131072)) Cn.add(s);
                else if (c & 2) {
                    var u = s;
                    (M?.delete(u),
                        c & 65536 ||
                            (c & 512 && (W === null || !(W.f & 2097152)) && (s.f |= oe),
                            Nn(u, b, n)));
                } else if (l) {
                    var d = s;
                    (c & 16 && N !== null && N.add(d), n === null ? yn(d) : n.push(d));
                }
            }
        }
}
function Pn(e) {
    if (typeof e != `object` || !e || ue in e) return e;
    let t = d(e);
    if (t !== l && t !== u) return e;
    var r = new Map(),
        i = n(e),
        a = On(0),
        o = null,
        c = Er,
        f = (e) => {
            if (Er === c) return e();
            var t = V,
                n = Er;
            (U(null), Dr(c));
            var r = e();
            return (U(t), Dr(n), r);
        };
    return (
        i && r.set(`length`, On(e.length, o)),
        new Proxy(e, {
            defineProperty(e, t, n) {
                (!(`value` in n) ||
                    n.configurable === !1 ||
                    n.enumerable === !1 ||
                    n.writable === !1) &&
                    Fe();
                var i = r.get(t);
                return (
                    i === void 0
                        ? f(() => {
                              var e = On(n.value, o);
                              return (r.set(t, e), e);
                          })
                        : P(i, n.value, !0),
                    !0
                );
            },
            deleteProperty(e, t) {
                var n = r.get(t);
                if (n === void 0) {
                    if (t in e) {
                        let e = f(() => On(C, o));
                        (r.set(t, e), Mn(a));
                    }
                } else (P(n, C), Mn(a));
                return !0;
            },
            get(t, n, i) {
                if (n === ue) return e;
                var a = r.get(n),
                    c = n in t;
                if (
                    (a === void 0 &&
                        (!c || s(t, n)?.writable) &&
                        ((a = f(() => On(Pn(c ? t[n] : C), o))), r.set(n, a)),
                    a !== void 0)
                ) {
                    var l = X(a);
                    return l === C ? void 0 : l;
                }
                return Reflect.get(t, n, i);
            },
            getOwnPropertyDescriptor(e, t) {
                var n = Reflect.getOwnPropertyDescriptor(e, t);
                if (n && `value` in n) {
                    var i = r.get(t);
                    i && (n.value = X(i));
                } else if (n === void 0) {
                    var a = r.get(t),
                        o = a?.v;
                    if (a !== void 0 && o !== C)
                        return { enumerable: !0, configurable: !0, value: o, writable: !0 };
                }
                return n;
            },
            has(e, t) {
                if (t === ue) return !0;
                var n = r.get(t),
                    i = (n !== void 0 && n.v !== C) || Reflect.has(e, t);
                return (n !== void 0 || (W !== null && (!i || s(e, t)?.writable))) &&
                    (n === void 0 && ((n = f(() => On(i ? Pn(e[t]) : C, o))), r.set(t, n)),
                    X(n) === C)
                    ? !1
                    : i;
            },
            set(e, t, n, c) {
                var l = r.get(t),
                    u = t in e;
                if (i && t === `length`)
                    for (var d = n; d < l.v; d += 1) {
                        var p = r.get(d + ``);
                        p === void 0
                            ? d in e && ((p = f(() => On(C, o))), r.set(d + ``, p))
                            : P(p, C);
                    }
                if (l === void 0)
                    (!u || s(e, t)?.writable) &&
                        ((l = f(() => On(void 0, o))), P(l, Pn(n)), r.set(t, l));
                else {
                    u = l.v !== C;
                    var m = f(() => Pn(n));
                    P(l, m);
                }
                var h = Reflect.getOwnPropertyDescriptor(e, t);
                if ((h?.set && h.set.call(c, n), !u)) {
                    if (i && typeof t == `string`) {
                        var g = r.get(`length`),
                            _ = Number(t);
                        Number.isInteger(_) && _ >= g.v && P(g, _ + 1);
                    }
                    Mn(a);
                }
                return !0;
            },
            ownKeys(e) {
                X(a);
                var t = Reflect.ownKeys(e).filter((e) => {
                    var t = r.get(e);
                    return t === void 0 || t.v !== C;
                });
                for (var [n, i] of r) i.v !== C && !(n in e) && t.push(n);
                return t;
            },
            setPrototypeOf() {
                Ie();
            },
        })
    );
}
var Fn, In, Ln, Rn;
function zn() {
    if (Fn === void 0) {
        ((Fn = window), (In = /Firefox/.test(navigator.userAgent)));
        var e = Element.prototype,
            t = Node.prototype,
            n = Text.prototype;
        ((Ln = s(t, `firstChild`).get),
            (Rn = s(t, `nextSibling`).get),
            f(e) && ((e[me] = void 0), (e[pe] = null), (e[he] = void 0), (e.__e = void 0)),
            f(n) && (n[ge] = void 0));
    }
}
function F(e = ``) {
    return document.createTextNode(e);
}
function I(e) {
    return Ln.call(e);
}
function L(e) {
    return Rn.call(e);
}
function Bn(e, t) {
    if (!w) return I(e);
    var n = I(E);
    if (n === null) n = E.appendChild(F());
    else if (t && n.nodeType !== 3) {
        var r = F();
        return (n?.before(r), D(r), r);
    }
    return (t && Kn(n), D(n), n);
}
function Vn(e, t = !1) {
    if (!w) {
        var n = I(e);
        return n instanceof Comment && n.data === `` ? L(n) : n;
    }
    if (t) {
        if (E?.nodeType !== 3) {
            var r = F();
            return (E?.before(r), D(r), r);
        }
        Kn(E);
    }
    return E;
}
function Hn(e, t = 1, n = !1) {
    let r = w ? E : e;
    for (var i; t--;) ((i = r), (r = L(r)));
    if (!w) return r;
    if (n) {
        if (r?.nodeType !== 3) {
            var a = F();
            return (r === null ? i?.after(a) : r.before(a), D(a), a);
        }
        Kn(r);
    }
    return (D(r), r);
}
function Un(e) {
    e.textContent = ``;
}
function Wn() {
    return !1;
}
function Gn(e, t, n) {
    return t == null || t === `http://www.w3.org/1999/xhtml`
        ? n
            ? document.createElement(e, { is: n })
            : document.createElement(e)
        : n
          ? document.createElementNS(t, e, { is: n })
          : document.createElementNS(t, e);
}
function Kn(e) {
    if (e.nodeValue.length < 65536) return;
    let t = e.nextSibling;
    for (; t !== null && t.nodeType === 3;)
        (t.remove(), (e.nodeValue += t.nodeValue), (t = e.nextSibling));
}
function qn(e) {
    (W === null && (V === null && De(e), Ee()), br && Te(e));
}
function Jn(e, t) {
    var n = t.last;
    n === null ? (t.last = t.first = e) : ((n.next = e), (e.prev = n), (t.last = e));
}
function R(e, t) {
    var n = W;
    n !== null && n.f & 8192 && (e |= x);
    var r = {
        ctx: O,
        deps: null,
        nodes: null,
        f: e | y | 512,
        first: null,
        fn: t,
        last: null,
        next: null,
        parent: n,
        b: n && n.b,
        prev: null,
        teardown: null,
        wv: 0,
        ac: null,
    };
    j?.register_created_effect(r);
    var i = r;
    if (e & 4) un === null ? mn.ensure().schedule(r) : un.push(r);
    else if (t !== null) {
        try {
            Pr(r);
        } catch (e) {
            throw (B(r), e);
        }
        i.deps === null &&
            i.teardown === null &&
            i.nodes === null &&
            i.first === i.last &&
            !(i.f & 524288) &&
            ((i = i.first), e & 16 && e & 65536 && i !== null && (i.f |= ne));
    }
    if (
        i !== null &&
        ((i.parent = n), n !== null && Jn(i, n), V !== null && V.f & 2 && !(e & 64))
    ) {
        var a = V;
        (a.effects ??= []).push(i);
    }
    return r;
}
function Yn() {
    return V !== null && !H;
}
function Xn(e) {
    let t = R(8, null);
    return (A(t, v), (t.teardown = e), t);
}
function Zn(e) {
    qn(`$effect`);
    var t = W.f;
    if (!V && t & 32 && O !== null && !O.i) {
        var n = O;
        (n.e ??= []).push(e);
    } else return Qn(e);
}
function Qn(e) {
    return R(4 | ie, e);
}
function $n(e) {
    return (qn(`$effect.pre`), R(8 | ie, e));
}
function er(e) {
    mn.ensure();
    let t = R(64 | re, e);
    return (e = {}) =>
        new Promise((n) => {
            e.outro
                ? pr(t, () => {
                      (B(t), n(void 0));
                  })
                : (B(t), n(void 0));
        });
}
function tr(e) {
    return R(4, e);
}
function nr(e, t) {
    var n = O,
        r = { effect: null, ran: !1, deps: e };
    (n.l.$.push(r),
        (r.effect = ar(() => {
            if ((e(), !r.ran)) {
                r.ran = !0;
                var n = W;
                try {
                    (G(n.parent), Z(t));
                } finally {
                    G(n);
                }
            }
        })));
}
function rr() {
    var e = O;
    ar(() => {
        for (var t of e.l.$) {
            t.deps();
            var n = t.effect;
            (n.f & 1024 && n.deps !== null && A(n, b), kr(n) && Pr(n), (t.ran = !1));
        }
    });
}
function ir(e) {
    return R(ce | re, e);
}
function ar(e, t = 0) {
    return R(8 | t, e);
}
function or(e, t = [], n = [], r = []) {
    Wt(r, t, n, (t) => {
        R(8, () => {
            e(...t.map(X));
        });
    });
}
function sr(e, t = 0) {
    return R(16 | t, e);
}
function z(e) {
    return R(32 | re, e);
}
function cr(e) {
    var t = e.teardown;
    if (t !== null) {
        let e = br,
            n = V;
        (xr(!0), U(null));
        try {
            t.call(null);
        } finally {
            (xr(e), U(n));
        }
    }
}
function lr(e, t = !1) {
    var n = e.first;
    for (e.first = e.last = null; n !== null;) {
        let e = n.ac;
        e !== null &&
            Rt(() => {
                e.abort(ve);
            });
        var r = n.next;
        (n.f & 64 ? (n.parent = null) : B(n, t), (n = r));
    }
}
function ur(e) {
    for (var t = e.first; t !== null;) {
        var n = t.next;
        (t.f & 32 || B(t), (t = n));
    }
}
function B(e, t = !0) {
    var n = !1;
    ((t || e.f & 262144) &&
        e.nodes !== null &&
        e.nodes.end !== null &&
        (dr(e.nodes.start, e.nodes.end), (n = !0)),
        (e.f |= te),
        lr(e, t && !n),
        Nr(e, 0));
    var r = e.nodes && e.nodes.t;
    if (r !== null) for (let e of r) e.stop();
    (cr(e), (e.f ^= te), (e.f |= S));
    var i = e.parent;
    (i !== null && i.first !== null && fr(e),
        (e.next = e.prev = e.teardown = e.ctx = e.deps = e.fn = e.nodes = e.ac = e.b = null));
}
function dr(e, t) {
    for (; e !== null;) {
        var n = e === t ? null : L(e);
        (e.remove(), (e = n));
    }
}
function fr(e) {
    var t = e.parent,
        n = e.prev,
        r = e.next;
    (n !== null && (n.next = r),
        r !== null && (r.prev = n),
        t !== null && (t.first === e && (t.first = r), t.last === e && (t.last = n)));
}
function pr(e, t, n = !0) {
    var r = [];
    mr(e, r, !0);
    var i = () => {
            (n && B(e), t && t());
        },
        a = r.length;
    if (a > 0) {
        var o = () => --a || i();
        for (var s of r) s.out(o);
    } else i();
}
function mr(e, t, n) {
    if (!(e.f & 8192)) {
        e.f ^= x;
        var r = e.nodes && e.nodes.t;
        if (r !== null) for (let e of r) (e.is_global || n) && t.push(e);
        for (var i = e.first; i !== null;) {
            var a = i.next;
            if (!(i.f & 64)) {
                var o = !!(i.f & 65536) || (!!(i.f & 32) && !!(e.f & 16));
                mr(i, t, o ? n : !1);
            }
            i = a;
        }
    }
}
function hr(e) {
    gr(e, !0);
}
function gr(e, t) {
    if (e.f & 8192) {
        ((e.f ^= x), e.f & 1024 || (A(e, y), mn.ensure().schedule(e)));
        for (var n = e.first; n !== null;) {
            var r = n.next,
                i = !!(n.f & 65536) || !!(n.f & 32);
            (gr(n, i ? t : !1), (n = r));
        }
        var a = e.nodes && e.nodes.t;
        if (a !== null) for (let e of a) (e.is_global || t) && e.in();
    }
}
function _r(e, t) {
    if (e.nodes)
        for (var n = e.nodes.start, r = e.nodes.end; n !== null;) {
            var i = n === r ? null : L(n);
            (t.append(n), (n = i));
        }
}
var vr = null,
    yr = !1,
    br = !1;
function xr(e) {
    br = e;
}
var V = null,
    H = !1;
function U(e) {
    V = e;
}
var W = null;
function G(e) {
    W = e;
}
var K = null;
function Sr(e) {
    V !== null && (K ??= new Set()).add(e);
}
var q = null,
    J = 0,
    Y = null;
function Cr(e) {
    Y = e;
}
var wr = 1,
    Tr = 0,
    Er = Tr;
function Dr(e) {
    Er = e;
}
function Or() {
    return ++wr;
}
function kr(e) {
    var t = e.f;
    if (t & 2048) return !0;
    if ((t & 2 && (e.f &= ~oe), t & 4096)) {
        for (var n = e.deps, r = n.length, i = 0; i < r; i++) {
            var a = n[i];
            if ((kr(a) && tn(a), a.wv > e.wv)) return !0;
        }
        t & 512 && M === null && A(e, v);
    }
    return !1;
}
function Ar(e, t, n = !0) {
    var r = e.reactions;
    if (r !== null && !(K !== null && K.has(e)))
        for (var i = 0; i < r.length; i++) {
            var a = r[i];
            a.f & 2 ? Ar(a, t, !1) : t === a && (n ? A(a, y) : a.f & 1024 && A(a, b), yn(a));
        }
}
function jr(e) {
    var t = q,
        n = J,
        r = Y,
        i = V,
        a = K,
        o = O,
        s = H,
        c = Er,
        l = e.f;
    ((q = null),
        (J = 0),
        (Y = null),
        (V = l & 96 ? null : e),
        (K = null),
        rt(e.ctx),
        (H = !1),
        (Er = ++Tr),
        e.ac !== null &&
            (Rt(() => {
                e.ac.abort(ve);
            }),
            (e.ac = null)));
    try {
        e.f |= se;
        var u = e.fn,
            d = u();
        e.f |= ee;
        var f = e.deps,
            p = j?.is_fork;
        if (q !== null) {
            var m;
            if ((p || Nr(e, J), f !== null && J > 0))
                for (f.length = J + q.length, m = 0; m < q.length; m++) f[J + m] = q[m];
            else e.deps = f = q;
            if (Yn() && e.f & 512) for (m = J; m < f.length; m++) (f[m].reactions ??= []).push(e);
        } else !p && f !== null && J < f.length && (Nr(e, J), (f.length = J));
        if (dt() && Y !== null && !H && f !== null && !(e.f & 6146))
            for (m = 0; m < Y.length; m++) Ar(Y[m], e);
        if (i !== null && i !== e) {
            if ((Tr++, i.deps !== null)) for (let e = 0; e < n; e += 1) i.deps[e].rv = Tr;
            if (t !== null) for (let e of t) e.rv = Tr;
            Y !== null && (r === null ? (r = Y) : r.push(...Y));
        }
        return (e.f & 8388608 && (e.f ^= le), d);
    } catch (e) {
        return _t(e);
    } finally {
        ((e.f ^= se), (q = t), (J = n), (Y = r), (V = i), (K = a), rt(o), (H = s), (Er = c));
    }
}
function Mr(e, t) {
    let n = t.reactions;
    if (n !== null) {
        var a = r.call(n, e);
        if (a !== -1) {
            var o = n.length - 1;
            o === 0 ? (n = t.reactions = null) : ((n[a] = n[o]), n.pop());
        }
    }
    if (n === null && t.f & 2 && (q === null || !i.call(q, t))) {
        var s = t;
        (s.f & 512 && ((s.f ^= 512), (s.f &= ~oe)),
            s.v !== C && bt(s),
            s.ac !== null &&
                Rt(() => {
                    (s.ac.abort(ve), (s.ac = null), A(s, y));
                }),
            nn(s),
            Nr(s, 0));
    }
}
function Nr(e, t) {
    var n = e.deps;
    if (n !== null) for (var r = t; r < n.length; r++) Mr(e, n[r]);
}
function Pr(e) {
    var t = e.f;
    if (!(t & 16384)) {
        A(e, v);
        var n = W,
            r = yr;
        ((W = e), (yr = !(t & 96)));
        try {
            (t & 16777232 ? ur(e) : lr(e), cr(e));
            var i = jr(e);
            ((e.teardown = typeof i == `function` ? i : null), (e.wv = wr));
        } finally {
            ((yr = r), (W = n));
        }
    }
}
async function Fr() {
    (await Promise.resolve(), hn());
}
function Ir() {
    return mn.ensure().settled();
}
function X(e) {
    var t = !!(e.f & 2);
    if (
        (vr?.add(e), V !== null && !H && !(W !== null && W.f & 16384) && (K === null || !K.has(e)))
    ) {
        var n = V.deps;
        if (V.f & 2097152)
            e.rv < Tr &&
                ((e.rv = Tr),
                q === null && n !== null && n[J] === e ? J++ : q === null ? (q = [e]) : q.push(e));
        else {
            ((V.deps ??= []), i.call(V.deps, e) || V.deps.push(e));
            var r = e.reactions;
            r === null ? (e.reactions = [V]) : i.call(r, V) || r.push(V);
        }
    }
    if (br && wn.has(e)) return wn.get(e);
    if (t) {
        var a = e;
        if (br) {
            var o = a.v;
            return (
                ((!(a.f & 1024) && a.reactions !== null) || Rr(a)) && (o = en(a)),
                wn.set(a, o),
                o
            );
        }
        var s = !(a.f & 512) && !H && V !== null && (yr || !!(V.f & 512)),
            c = (a.f & ee) === 0;
        (kr(a) && (s && (a.f |= 512), tn(a)), s && !c && (rn(a), Lr(a)));
    }
    if (M?.has(e)) return M.get(e);
    if (e.f & 8388608) throw e.v;
    return e.v;
}
function Lr(e) {
    if (((e.f |= 512), e.deps !== null))
        for (let t of e.deps)
            ((t.reactions ??= []).push(e), t.f & 2 && !(t.f & 512) && (rn(t), Lr(t)));
}
function Rr(e) {
    if (e.v === C) return !0;
    if (e.deps === null) return !1;
    for (let t of e.deps) if (wn.has(t) || (t.f & 2 && Rr(t))) return !0;
    return !1;
}
function Z(e) {
    var t = H;
    try {
        return ((H = !0), e());
    } finally {
        H = t;
    }
}
function zr(e) {
    if (!(typeof e != `object` || !e || e instanceof EventTarget)) {
        if (ue in e) Br(e);
        else if (!Array.isArray(e))
            for (let t in e) {
                let n = e[t];
                typeof n == `object` && n && ue in n && Br(n);
            }
    }
}
function Br(e, t = new Set()) {
    if (typeof e == `object` && e && !(e instanceof EventTarget) && !t.has(e)) {
        (t.add(e), e instanceof Date && e.getTime());
        for (let n in e)
            try {
                Br(e[n], t);
            } catch {}
        let n = d(e);
        if (
            n !== Object.prototype &&
            n !== Array.prototype &&
            n !== Map.prototype &&
            n !== Set.prototype &&
            n !== Date.prototype
        ) {
            let t = c(n);
            for (let n in t) {
                let r = t[n].get;
                if (r)
                    try {
                        r.call(e);
                    } catch {}
            }
        }
    }
}
[
    ...`allowfullscreen.async.autofocus.autoplay.checked.controls.default.disabled.formnovalidate.indeterminate.inert.ismap.loop.multiple.muted.nomodule.novalidate.open.playsinline.readonly.required.reversed.seamless.selected.webkitdirectory.defer.disablepictureinpicture.disableremoteplayback`.split(
        `.`
    ),
];
var Vr = [`touchstart`, `touchmove`];
function Hr(e) {
    return Vr.includes(e);
}
var Ur = Symbol(`events`),
    Wr = new Set(),
    Gr = new Set();
function Kr(e, t, n, r = {}) {
    function i(e) {
        if ((r.capture || Zr.call(t, e), !e.cancelBubble)) return Rt(() => n?.call(this, e));
    }
    return (
        e.startsWith(`pointer`) || e.startsWith(`touch`) || e === `wheel`
            ? k(() => {
                  t.addEventListener(e, i, r);
              })
            : t.addEventListener(e, i, r),
        i
    );
}
function qr(e, t, n, r, i) {
    var a = { capture: r, passive: i },
        o = Kr(e, t, n, a);
    (t === document.body || t === window || t === document || t instanceof HTMLMediaElement) &&
        Xn(() => {
            t.removeEventListener(e, o, a);
        });
}
function Jr(e, t, n) {
    (t[Ur] ??= {})[e] = n;
}
function Yr(e) {
    for (var t = 0; t < e.length; t++) Wr.add(e[t]);
    for (var n of Gr) n(e);
}
var Xr = null;
function Zr(e) {
    var t = this,
        n = t.ownerDocument,
        r = e.type,
        i = e.composedPath?.() || [],
        a = i[0] || e.target;
    Xr = e;
    var s = 0,
        c = Xr === e && e[Ur];
    if (c) {
        var l = i.indexOf(c);
        if (l !== -1 && (t === document || t === window)) {
            e[Ur] = t;
            return;
        }
        var u = i.indexOf(t);
        if (u === -1) return;
        l <= u && (s = l);
    }
    if (((a = i[s] || e.target), a !== t)) {
        o(e, `currentTarget`, {
            configurable: !0,
            get() {
                return a || n;
            },
        });
        var d = V,
            f = W;
        (U(null), G(null));
        try {
            for (var p, m = []; a !== null && a !== t;) {
                try {
                    var h = a[Ur]?.[r];
                    h != null && (!a.disabled || e.target === a) && h.call(a, e);
                } catch (e) {
                    p ? m.push(e) : (p = e);
                }
                if (e.cancelBubble) break;
                (s++, (a = s < i.length ? i[s] : null));
            }
            if (p) {
                for (let e of m)
                    queueMicrotask(() => {
                        throw e;
                    });
                throw p;
            }
        } finally {
            ((e[Ur] = t), delete e.currentTarget, U(d), G(f));
        }
    }
}
var Qr =
    globalThis?.window?.trustedTypes &&
    globalThis.window.trustedTypes.createPolicy(`svelte-trusted-html`, { createHTML: (e) => e });
function $r(e) {
    return Qr?.createHTML(e) ?? e;
}
function ei(e) {
    var t = Gn(`template`);
    return ((t.innerHTML = $r(e.replaceAll(`<!>`, `<!---->`))), t.content);
}
function Q(e, t) {
    var n = W;
    n.nodes === null && (n.nodes = { start: e, end: t, a: null, t: null });
}
function ti(e, t) {
    var n = !!(t & 1),
        r = !!(t & 2),
        i,
        a = !e.startsWith(`<!>`);
    return () => {
        if (w) return (Q(E, null), E);
        i === void 0 && ((i = ei(a ? e : `<!>` + e)), n || (i = I(i)));
        var t = r || In ? document.importNode(i, !0) : i.cloneNode(!0);
        if (n) {
            var o = I(t),
                s = t.lastChild;
            Q(o, s);
        } else Q(t, t);
        return t;
    };
}
function ni(e, t, n = `svg`) {
    var r = !e.startsWith(`<!>`),
        i = !!(t & 1),
        a = `<${n}>${r ? e : `<!>` + e}</${n}>`,
        o;
    return () => {
        if (w) return (Q(E, null), E);
        if (!o) {
            var e = I(ei(a));
            if (i) for (o = document.createDocumentFragment(); I(e);) o.appendChild(I(e));
            else o = I(e);
        }
        var t = o.cloneNode(!0);
        if (i) {
            var n = I(t),
                r = t.lastChild;
            Q(n, r);
        } else Q(t, t);
        return t;
    };
}
function ri(e, t) {
    return ni(e, t, `svg`);
}
function ii(e = ``) {
    if (!w) {
        var t = F(e + ``);
        return (Q(t, t), t);
    }
    var n = E;
    return (n.nodeType === 3 ? Kn(n) : (n.before((n = F())), D(n)), Q(n, n), n);
}
function ai() {
    if (w) return (Q(E, null), E);
    var e = document.createDocumentFragment(),
        t = document.createComment(``),
        n = F();
    return (e.append(t, n), Q(t, n), e);
}
function oi(e, t) {
    if (w) {
        var n = W;
        ((!(n.f & 32768) || n.nodes.end === null) && (n.nodes.end = E), qe());
        return;
    }
    e !== null && e.before(t);
}
var si = !0;
function ci(e, t) {
    var n = t == null ? `` : typeof t == `object` ? `${t}` : t;
    n !== (e[ge] ??= e.nodeValue) && ((e[ge] = n), (e.nodeValue = `${n}`));
}
function li(e, t) {
    return fi(e, t);
}
function ui(e, t) {
    (zn(), (t.intro = t.intro ?? !1));
    let n = t.target,
        r = w,
        i = E;
    try {
        for (var a = I(n); a && (a.nodeType !== 8 || a.data !== `[`);) a = L(a);
        if (!a) throw ze;
        (T(!0), D(a));
        let r = fi(e, { ...t, anchor: a });
        return (T(!1), r);
    } catch (r) {
        if (
            r instanceof Error &&
            r.message
                .split(
                    `
`
                )
                .some((e) => e.startsWith(`https://svelte.dev/e/`))
        )
            throw r;
        return (
            r !== ze && console.warn(`Failed to hydrate: `, r),
            t.recover === !1 && Me(),
            zn(),
            Un(n),
            T(!1),
            li(e, t)
        );
    } finally {
        (T(r), D(i));
    }
}
var di = new Map();
function fi(
    e,
    { target: t, anchor: n, props: r = {}, events: i, context: o, intro: s = !0, transformError: c }
) {
    zn();
    var l = void 0,
        u = er(() => {
            var u = n ?? t.appendChild(F());
            Ht(
                u,
                { pending: () => {} },
                (t) => {
                    lt({});
                    var n = O;
                    if (
                        (o && (n.c = o),
                        i && (r.$$events = i),
                        w && Q(t, null),
                        (si = s),
                        (l = e(t, r) || {}),
                        (si = !0),
                        w && ((W.nodes.end = E), E === null || E.nodeType !== 8 || E.data !== `]`))
                    )
                        throw (Ge(), ze);
                    ut();
                },
                c
            );
            var d = new Set(),
                f = (e) => {
                    for (var n = 0; n < e.length; n++) {
                        var r = e[n];
                        if (!d.has(r)) {
                            d.add(r);
                            var i = Hr(r);
                            for (let e of [t, document]) {
                                var a = di.get(e);
                                a === void 0 && ((a = new Map()), di.set(e, a));
                                var o = a.get(r);
                                o === void 0
                                    ? (e.addEventListener(r, Zr, { passive: i }), a.set(r, 1))
                                    : a.set(r, o + 1);
                            }
                        }
                    }
                };
            return (
                f(a(Wr)),
                Gr.add(f),
                () => {
                    for (var e of d)
                        for (let n of [t, document]) {
                            var r = di.get(n),
                                i = r.get(e);
                            --i == 0
                                ? (n.removeEventListener(e, Zr),
                                  r.delete(e),
                                  r.size === 0 && di.delete(n))
                                : r.set(e, i);
                        }
                    (Gr.delete(f), u !== n && u.parentNode?.removeChild(u));
                }
            );
        });
    return (pi.set(l, u), l);
}
var pi = new WeakMap();
function mi(e, t) {
    let n = pi.get(e);
    return n ? (pi.delete(e), n(t)) : Promise.resolve();
}
var hi = class {
    anchor;
    #e = new Map();
    #t = new Map();
    #n = new Map();
    #r = new Set();
    #i = !0;
    constructor(e, t = !0) {
        ((this.anchor = e), (this.#i = t));
    }
    #a = (e) => {
        if (this.#e.has(e)) {
            var t = this.#e.get(e),
                n = this.#t.get(t);
            if (n) (hr(n), this.#r.delete(t));
            else {
                var r = this.#n.get(t);
                r &&
                    (hr(r.effect),
                    this.#t.set(t, r.effect),
                    this.#n.delete(t),
                    r.fragment.lastChild.remove(),
                    this.anchor.before(r.fragment),
                    (n = r.effect));
            }
            for (let [t, n] of this.#e) {
                if ((this.#e.delete(t), t === e)) break;
                let r = this.#n.get(n);
                r && (B(r.effect), this.#n.delete(n));
            }
            for (let [e, r] of this.#t) {
                if (e === t || this.#r.has(e)) continue;
                let i = () => {
                    if (Array.from(this.#e.values()).includes(e)) {
                        var t = document.createDocumentFragment();
                        (_r(r, t), t.append(F()), this.#n.set(e, { effect: r, fragment: t }));
                    } else B(r);
                    (this.#r.delete(e), this.#t.delete(e));
                };
                this.#i || !n ? (this.#r.add(e), pr(r, i, !1)) : i();
            }
        }
    };
    #o = (e) => {
        this.#e.delete(e);
        let t = Array.from(this.#e.values());
        for (let [e, n] of this.#n) t.includes(e) || (B(n.effect), this.#n.delete(e));
    };
    ensure(e, t) {
        var n = j,
            r = Wn();
        if (t && !this.#t.has(e) && !this.#n.has(e)) {
            if (r) {
                var i = document.createDocumentFragment(),
                    a = F();
                (i.append(a), this.#n.set(e, { effect: z(() => t(a)), fragment: i }));
            } else
                this.#t.set(
                    e,
                    z(() => t(this.anchor))
                );
        }
        if ((this.#e.set(n, e), r)) {
            for (let [t, r] of this.#t) t === e ? n.unskip_effect(r) : n.skip_effect(r);
            for (let [t, r] of this.#n)
                t === e ? n.unskip_effect(r.effect) : n.skip_effect(r.effect);
            (n.oncommit(this.#a), n.ondiscard(this.#o));
        } else (w && (this.anchor = E), this.#a(n));
    }
};
function gi(e, t, n = !1) {
    var r;
    w && ((r = E), qe());
    var i = new hi(e),
        a = n ? ne : 0;
    function o(e, t) {
        if (w) {
            var n = Ze(r);
            if (e !== parseInt(n.substring(1))) {
                var a = Xe();
                (D(a), (i.anchor = a), T(!1), i.ensure(e, t), T(!0));
                return;
            }
        }
        i.ensure(e, t);
    }
    sr(() => {
        var e = !1;
        (t((t, n = 0) => {
            ((e = !0), o(n, t));
        }),
            e || o(-1, null));
    }, a);
}
function _i(e, t) {
    return t;
}
function vi(e, t, n) {
    for (var r = [], i = t.length, o, s = t.length, c = 0; c < i; c++) {
        let n = t[c];
        pr(
            n,
            () => {
                if (o) {
                    if ((o.pending.delete(n), o.done.add(n), o.pending.size === 0)) {
                        var t = e.outrogroups;
                        (yi(e, a(o.done)), t.delete(o), t.size === 0 && (e.outrogroups = null));
                    }
                } else --s;
            },
            !1
        );
    }
    if (s === 0) {
        var l = r.length === 0 && n !== null;
        if (l) {
            var u = n,
                d = u.parentNode;
            (Un(d), d.append(u), e.items.clear());
        }
        yi(e, t, !l);
    } else ((o = { pending: new Set(t), done: new Set() }), (e.outrogroups ??= new Set()).add(o));
}
function yi(e, t, n = !0) {
    var r;
    if (e.pending.size > 0) {
        r = new Set();
        for (let t of e.pending.values()) for (let n of t) r.add(e.items.get(n).e);
    }
    for (var i = 0; i < t.length; i++) {
        var a = t[i];
        r?.has(a) ? ((a.f |= ae), _r(a, document.createDocumentFragment())) : B(t[i], n);
    }
}
var bi;
function xi(e, t, r, i, o, s = null) {
    var c = e,
        l = new Map();
    if (t & 4) {
        var u = e;
        c = w ? D(I(u)) : u.appendChild(F());
    }
    w && qe();
    var d = null,
        f = Qt(() => {
            var e = r();
            return n(e) ? e : e == null ? [] : a(e);
        }),
        p,
        m = new Map(),
        h = !0;
    function g(e) {
        v.effect.f & 16384 ||
            (v.pending.delete(e),
            (v.fallback = d),
            Ci(v, p, c, t, i),
            d !== null &&
                (p.length === 0
                    ? d.f & 33554432
                        ? ((d.f ^= ae), Ti(d, null, c))
                        : hr(d)
                    : pr(d, () => {
                          d = null;
                      })));
    }
    function _(e) {
        v.pending.delete(e);
    }
    var v = {
        effect: sr(() => {
            p = X(f);
            var e = p.length;
            let n = !1;
            w && (Ze(c) === `[!`) != (e === 0) && ((c = Xe()), D(c), T(!1), (n = !0));
            for (var a = new Set(), u = j, v = Wn(), y = 0; y < e; y += 1) {
                w && E.nodeType === 8 && E.data === `]` && ((c = E), (n = !0), T(!1));
                var b = p[y],
                    x = i(b, y),
                    S = h ? null : l.get(x);
                (S
                    ? (S.v && An(S.v, b), S.i && An(S.i, y), v && u.unskip_effect(S.e))
                    : ((S = wi(l, h ? c : (bi ??= F()), b, x, y, o, t, r)),
                      h || (S.e.f |= ae),
                      l.set(x, S)),
                    a.add(x));
            }
            if (
                (e === 0 &&
                    s &&
                    !d &&
                    (h ? (d = z(() => s(c))) : ((d = z(() => s((bi ??= F())))), (d.f |= ae))),
                e > a.size && we(``, ``, ``),
                w && e > 0 && D(Xe()),
                !h)
            ) {
                if ((m.set(u, a), v)) {
                    for (let [e, t] of l) a.has(e) || u.skip_effect(t.e);
                    (u.oncommit(g), u.ondiscard(_));
                } else g(u);
            }
            (n && T(!0), X(f));
        }),
        flags: t,
        items: l,
        pending: m,
        outrogroups: null,
        fallback: d,
    };
    ((h = !1), w && (c = E));
}
function Si(e) {
    for (; e !== null && !(e.f & 32);) e = e.next;
    return e;
}
function Ci(e, t, n, r, i) {
    var o = !!(r & 8),
        s = t.length,
        c = e.items,
        l = Si(e.effect.first),
        u,
        d = null,
        f,
        p = [],
        m = [],
        h,
        g,
        _,
        v;
    if (o)
        for (v = 0; v < s; v += 1)
            ((h = t[v]),
                (g = i(h, v)),
                (_ = c.get(g).e),
                _.f & 33554432 || (_.nodes?.a?.measure(), (f ??= new Set()).add(_)));
    for (v = 0; v < s; v += 1) {
        if (((h = t[v]), (g = i(h, v)), (_ = c.get(g).e), e.outrogroups !== null))
            for (let t of e.outrogroups) (t.pending.delete(_), t.done.delete(_));
        if (
            (_.f & 8192 && (hr(_), o && (_.nodes?.a?.unfix(), (f ??= new Set()).delete(_))),
            _.f & 33554432)
        ) {
            if (((_.f ^= ae), _ === l)) Ti(_, null, n);
            else {
                var y = d ? d.next : l;
                (_ === e.effect.last && (e.effect.last = _.prev),
                    _.prev && (_.prev.next = _.next),
                    _.next && (_.next.prev = _.prev),
                    Ei(e, d, _),
                    Ei(e, _, y),
                    Ti(_, y, n),
                    (d = _),
                    (p = []),
                    (m = []),
                    (l = Si(d.next)));
                continue;
            }
        }
        if (_ !== l) {
            if (u !== void 0 && u.has(_)) {
                if (p.length < m.length) {
                    var b = m[0],
                        x;
                    d = b.prev;
                    var S = p[0],
                        ee = p[p.length - 1];
                    for (x = 0; x < p.length; x += 1) Ti(p[x], b, n);
                    for (x = 0; x < m.length; x += 1) u.delete(m[x]);
                    (Ei(e, S.prev, ee.next),
                        Ei(e, d, S),
                        Ei(e, ee, b),
                        (l = b),
                        (d = ee),
                        --v,
                        (p = []),
                        (m = []));
                } else
                    (u.delete(_),
                        Ti(_, l, n),
                        Ei(e, _.prev, _.next),
                        Ei(e, _, d === null ? e.effect.first : d.next),
                        Ei(e, d, _),
                        (d = _));
                continue;
            }
            for (p = [], m = []; l !== null && l !== _;)
                ((u ??= new Set()).add(l), m.push(l), (l = Si(l.next)));
            if (l === null) continue;
        }
        (_.f & 33554432 || p.push(_), (d = _), (l = Si(_.next)));
    }
    if (e.outrogroups !== null) {
        for (let t of e.outrogroups)
            t.pending.size === 0 && (yi(e, a(t.done)), e.outrogroups?.delete(t));
        e.outrogroups.size === 0 && (e.outrogroups = null);
    }
    if (l !== null || u !== void 0) {
        var te = [];
        if (u !== void 0) for (_ of u) _.f & 8192 || te.push(_);
        for (; l !== null;) (!(l.f & 8192) && l !== e.fallback && te.push(l), (l = Si(l.next)));
        var ne = te.length;
        if (ne > 0) {
            var re = r & 4 && s === 0 ? n : null;
            if (o) {
                for (v = 0; v < ne; v += 1) te[v].nodes?.a?.measure();
                for (v = 0; v < ne; v += 1) te[v].nodes?.a?.fix();
            }
            vi(e, te, re);
        }
    }
    o &&
        k(() => {
            if (f !== void 0) for (_ of f) _.nodes?.a?.apply();
        });
}
function wi(e, t, n, r, i, a, o, s) {
    var c = o & 1 ? (o & 16 ? Dn(n) : kn(n, !1, !1)) : null,
        l = o & 2 ? Dn(i) : null;
    return {
        v: c,
        i: l,
        e: z(
            () => (
                a(t, c ?? n, l ?? i, s),
                () => {
                    e.delete(r);
                }
            )
        ),
    };
}
function Ti(e, t, n) {
    if (e.nodes)
        for (
            var r = e.nodes.start, i = e.nodes.end, a = t && !(t.f & 33554432) ? t.nodes.start : n;
            r !== null;
        ) {
            var o = L(r);
            if ((a.before(r), r === i)) return;
            r = o;
        }
}
function Ei(e, t, n) {
    (t === null ? (e.effect.first = n) : (t.next = n),
        n === null ? (e.effect.last = t) : (n.prev = t));
}
function Di(e, t, n = !1, r = !1, i = !1, a = !1) {
    var o = e,
        s = ``;
    if (n) {
        var c = e;
        w && (o = D(I(c)));
    }
    or(() => {
        var e = W;
        if (s === (s = t() ?? ``)) {
            w && qe();
            return;
        }
        if (n && !w) {
            ((e.nodes = null), (c.innerHTML = s), s !== `` && Q(I(c), c.lastChild));
            return;
        }
        if ((e.nodes !== null && (dr(e.nodes.start, e.nodes.end), (e.nodes = null)), s !== ``)) {
            if (w) {
                for (
                    var a = E.data, l = qe(), u = l;
                    l !== null && (l.nodeType !== 8 || l.data !== ``);
                )
                    ((u = l), (l = L(l)));
                if (l === null) throw (Ge(), ze);
                (Q(E, u), (o = D(l)));
                return;
            }
            var d = Gn(r ? `svg` : i ? `math` : `template`, r ? Ve : i ? He : void 0);
            d.innerHTML = s;
            var f = r || i ? d : d.content;
            if ((Q(I(f), f.lastChild), r || i)) for (; I(f);) o.before(I(f));
            else o.before(f);
        }
    });
}
function Oi(e, t, ...n) {
    var r = new hi(e);
    sr(() => {
        let e = t() ?? null;
        r.ensure(e, e && ((t) => e(t, ...n)));
    }, ne);
}
function ki(e) {
    return (t, ...n) => {
        var r = e(...n),
            i;
        w ? ((i = E), qe()) : ((i = I(ei(r.render().trim()))), t.before(i));
        let a = r.setup?.(i);
        (Q(i, i), typeof a == `function` && Xn(a));
    };
}
function Ai(e, t, n) {
    var r;
    w && ((r = E), qe());
    var i = new hi(e);
    sr(() => {
        var e = t() ?? null;
        if (w && (Ze(r) === `[`) != (e !== null)) {
            var a = Xe();
            (D(a), (i.anchor = a), T(!1), i.ensure(e, e && ((t) => n(t, e))), T(!0));
            return;
        }
        i.ensure(e, e && ((t) => n(t, e)));
    }, ne);
}
var ji = () => performance.now(),
    $ = { tick: (e) => requestAnimationFrame(e), now: () => ji(), tasks: new Set() };
function Mi() {
    let e = $.now();
    ($.tasks.forEach((t) => {
        t.c(e) || ($.tasks.delete(t), t.f());
    }),
        $.tasks.size !== 0 && $.tick(Mi));
}
function Ni(e) {
    let t;
    return (
        $.tasks.size === 0 && $.tick(Mi),
        {
            promise: new Promise((n) => {
                $.tasks.add((t = { c: e, f: n }));
            }),
            abort() {
                $.tasks.delete(t);
            },
        }
    );
}
function Pi(e, t) {
    Rt(() => {
        e.dispatchEvent(new CustomEvent(t));
    });
}
function Fi(e) {
    if (e === `float`) return `cssFloat`;
    if (e === `offset`) return `cssOffset`;
    if (e.startsWith(`--`)) return e;
    let t = e.split(`-`);
    return t.length === 1
        ? t[0]
        : t[0] +
              t
                  .slice(1)
                  .map((e) => e[0].toUpperCase() + e.slice(1))
                  .join(``);
}
function Ii(e) {
    let t = {},
        n = e.split(`;`);
    for (let e of n) {
        let [n, r] = e.split(`:`);
        if (!n || r === void 0) break;
        let i = Fi(n.trim());
        t[i] = r.trim();
    }
    return t;
}
var Li = (e) => e,
    Ri = null;
function zi(e, t, n) {
    var r = (Ri ?? W).nodes,
        i,
        a,
        o,
        s = null;
    ((r.a ??= {
        element: e,
        measure() {
            i = this.element.getBoundingClientRect();
        },
        apply() {
            if (
                (o?.abort(),
                (a = this.element.getBoundingClientRect()),
                i.left !== a.left ||
                    i.right !== a.right ||
                    i.top !== a.top ||
                    i.bottom !== a.bottom)
            ) {
                let e = t()(this.element, { from: i, to: a }, n?.());
                o = Vi(
                    this.element,
                    e,
                    void 0,
                    1,
                    () => {},
                    () => {
                        (o?.abort(), (o = void 0));
                    }
                );
            }
        },
        fix() {
            if (!e.getAnimations().length) {
                var { position: t, width: n, height: r } = getComputedStyle(e);
                if (t !== `absolute` && t !== `fixed`) {
                    var a = e.style;
                    ((s = {
                        position: a.position,
                        width: a.width,
                        height: a.height,
                        transform: a.transform,
                    }),
                        (a.position = `absolute`),
                        (a.width = n),
                        (a.height = r));
                    var o = e.getBoundingClientRect();
                    if (i.left !== o.left || i.top !== o.top) {
                        var c = `translate(${i.left - o.left}px, ${i.top - o.top}px)`;
                        a.transform = a.transform ? `${a.transform} ${c}` : c;
                    }
                }
            }
        },
        unfix() {
            if (s) {
                var t = e.style;
                ((t.position = s.position),
                    (t.width = s.width),
                    (t.height = s.height),
                    (t.transform = s.transform));
            }
        },
    }),
        (r.a.element = e));
}
function Bi(e, t, n, r) {
    var i = !!(e & 1),
        a = !!(e & 2),
        o = i && a,
        s = !!(e & 4),
        c = o ? `both` : i ? `in` : `out`,
        l,
        u = t.inert,
        d = t.style.overflow,
        f,
        p;
    function m() {
        return Rt(() => (l ??= n()(t, r?.() ?? {}, { direction: c })));
    }
    var h = {
            is_global: s,
            in() {
                if (((t.inert = u), !i)) {
                    (p?.abort(), p?.reset?.());
                    return;
                }
                (a || f?.abort(),
                    (f = Vi(
                        t,
                        m(),
                        p,
                        1,
                        () => {
                            Pi(t, `introstart`);
                        },
                        () => {
                            (Pi(t, `introend`),
                                f?.abort(),
                                (f = l = void 0),
                                (t.style.overflow = d));
                        }
                    )));
            },
            out(e) {
                if (!a) {
                    (e?.(), (l = void 0));
                    return;
                }
                ((t.inert = !0),
                    (p = Vi(
                        t,
                        m(),
                        f,
                        0,
                        () => {
                            Pi(t, `outrostart`);
                        },
                        () => {
                            (Pi(t, `outroend`), e?.());
                        }
                    )));
            },
            stop: () => {
                (f?.abort(), p?.abort());
            },
        },
        g = W;
    if (((g.nodes.t ??= []).push(h), i && si)) {
        var _ = s;
        if (!_) {
            for (var v = g.parent; v && v.f & 65536;) for (; (v = v.parent) && !(v.f & 16););
            _ = !v || !!(v.f & 32768);
        }
        _ &&
            tr(() => {
                Z(() => h.in());
            });
    }
}
function Vi(e, t, n, r, i, a) {
    var o = r === 1;
    if (p(t)) {
        var s,
            c = !1;
        return (
            k(() => {
                c || (s = Vi(e, t({ direction: o ? `in` : `out` }), n, r, i, a));
            }),
            {
                abort: () => {
                    ((c = !0), s?.abort());
                },
                deactivate: () => s.deactivate(),
                reset: () => s.reset(),
                t: () => s.t(),
            }
        );
    }
    if ((n?.deactivate(), !t?.duration && !t?.delay))
        return (i(), a(), { abort: m, deactivate: m, reset: m, t: () => r });
    let { delay: l = 0, css: u, tick: d, easing: f = Li } = t;
    var h = [];
    if (o && n === void 0 && (d && d(0, 1), u)) {
        var g = Ii(u(0, 1));
        h.push(g, g);
    }
    var _ = () => 1 - r,
        v = e.animate(h, { duration: l, fill: `forwards` });
    return (
        (v.onfinish = () => {
            (v.cancel(), i());
            var o = n?.t() ?? 1 - r;
            n?.abort();
            var s = r - o,
                c = t.duration * Math.abs(s),
                l = [];
            if (c > 0) {
                var p = !1;
                if (u)
                    for (var m = Math.ceil(c / (1e3 / 60)), h = 0; h <= m; h += 1) {
                        var g = o + s * f(h / m),
                            y = Ii(u(g, 1 - g));
                        (l.push(y), (p ||= y.overflow === `hidden`));
                    }
                (p && (e.style.overflow = `hidden`),
                    (_ = () => {
                        var e = v.currentTime;
                        return o + s * f(e / c);
                    }),
                    d &&
                        Ni(() => {
                            if (v.playState !== `running`) return !1;
                            var e = _();
                            return (d(e, 1 - e), !0);
                        }));
            }
            ((v = e.animate(l, { duration: c, fill: `forwards` })),
                (v.onfinish = () => {
                    ((_ = () => r), d?.(r, 1 - r), a());
                }));
        }),
        {
            abort: () => {
                v && (v.cancel(), (v.effect = null), (v.onfinish = m));
            },
            deactivate: () => {
                a = m;
            },
            reset: () => {
                r === 0 && d?.(1, 0);
            },
            t: () => _(),
        }
    );
}
function Hi(e) {
    var t,
        n,
        r = ``;
    if (typeof e == `string` || typeof e == `number`) r += e;
    else if (typeof e == `object`) {
        if (Array.isArray(e)) {
            var i = e.length;
            for (t = 0; t < i; t++) e[t] && (n = Hi(e[t])) && (r && (r += ` `), (r += n));
        } else for (n in e) e[n] && (r && (r += ` `), (r += n));
    }
    return r;
}
function Ui() {
    for (var e, t, n = 0, r = ``, i = arguments.length; n < i; n++)
        (e = arguments[n]) && (t = Hi(e)) && (r && (r += ` `), (r += t));
    return r;
}
function Wi(e) {
    return typeof e == `object` ? Ui(e) : (e ?? ``);
}
var Gi = [
    ...` 	
\r\f\xA0\v﻿`,
];
function Ki(e, t, n) {
    var r = e == null ? `` : `` + e;
    if ((t && (r = r ? r + ` ` + t : t), n)) {
        for (var i of Object.keys(n))
            if (n[i]) r = r ? r + ` ` + i : i;
            else if (r.length)
                for (var a = i.length, o = 0; (o = r.indexOf(i, o)) >= 0;) {
                    var s = o + a;
                    (o === 0 || Gi.includes(r[o - 1])) && (s === r.length || Gi.includes(r[s]))
                        ? (r = (o === 0 ? `` : r.substring(0, o)) + r.substring(s + 1))
                        : (o = s);
                }
    }
    return r === `` ? null : r;
}
function qi(e, t = !1) {
    var n = t ? ` !important;` : `;`,
        r = ``;
    for (var i of Object.keys(e)) {
        var a = e[i];
        a != null && a !== `` && (r += ` ` + i + `: ` + a + n);
    }
    return r;
}
function Ji(e) {
    return e[0] !== `-` || e[1] !== `-` ? e.toLowerCase() : e;
}
function Yi(e, t) {
    if (t) {
        var n = ``,
            r,
            i;
        if ((Array.isArray(t) ? ((r = t[0]), (i = t[1])) : (r = t), e)) {
            e = String(e)
                .replaceAll(/\s*\/\*.*?\*\/\s*/g, ``)
                .trim();
            var a = !1,
                o = 0,
                s = !1,
                c = [];
            (r && c.push(...Object.keys(r).map(Ji)), i && c.push(...Object.keys(i).map(Ji)));
            var l = 0,
                u = -1;
            let t = e.length;
            for (var d = 0; d < t; d++) {
                var f = e[d];
                if (
                    (s
                        ? f === `/` && e[d - 1] === `*` && (s = !1)
                        : a
                          ? a === f && (a = !1)
                          : f === `/` && e[d + 1] === `*`
                            ? (s = !0)
                            : f === `"` || f === `'`
                              ? (a = f)
                              : f === `(`
                                ? o++
                                : f === `)` && o--,
                    !s && a === !1 && o === 0)
                ) {
                    if (f === `:` && u === -1) u = d;
                    else if (f === `;` || d === t - 1) {
                        if (u !== -1) {
                            var p = Ji(e.substring(l, u).trim());
                            if (!c.includes(p)) {
                                f !== `;` && d++;
                                var m = e.substring(l, d).trim();
                                n += ` ` + m + `;`;
                            }
                        }
                        ((l = d + 1), (u = -1));
                    }
                }
            }
        }
        return (r && (n += qi(r)), i && (n += qi(i, !0)), (n = n.trim()), n === `` ? null : n);
    }
    return e == null ? null : String(e);
}
function Xi(e, t, n, r, i, a) {
    var o = e[me];
    if (w || o !== n || o === void 0) {
        var s = Ki(n, r, a);
        ((!w || s !== e.getAttribute(`class`)) &&
            (s == null
                ? e.removeAttribute(`class`)
                : t
                  ? (e.className = s)
                  : e.setAttribute(`class`, s)),
            (e[me] = n));
    } else if (a && i !== a)
        for (var c in a) {
            var l = !!a[c];
            (i == null || l !== !!i[c]) && e.classList.toggle(c, l);
        }
    return a;
}
function Zi(e, t = {}, n, r) {
    for (var i in n) {
        var a = n[i];
        t[i] !== a && (n[i] == null ? e.style.removeProperty(i) : e.style.setProperty(i, a, r));
    }
}
function Qi(e, t, n, r) {
    var i = e[he];
    if (w || i !== t) {
        var a = Yi(t, r);
        ((!w || a !== e.getAttribute(`style`)) &&
            (a == null ? e.removeAttribute(`style`) : (e.style.cssText = a)),
            (e[he] = t));
    } else
        r &&
            (Array.isArray(r)
                ? (Zi(e, n?.[0], r[0]), Zi(e, n?.[1], r[1], `important`))
                : Zi(e, n, r));
    return r;
}
var $i = Symbol(`is custom element`),
    ea = Symbol(`is html`),
    ta = ye ? `link` : `LINK`,
    na = ye ? `progress` : `PROGRESS`;
function ra(e) {
    if (w) {
        var t = !1,
            n = () => {
                if (!t) {
                    if (((t = !0), e.hasAttribute(`value`))) {
                        var n = e.value;
                        (aa(e, `value`, null), (e.value = n));
                    }
                    if (e.hasAttribute(`checked`)) {
                        var r = e.checked;
                        (aa(e, `checked`, null), (e.checked = r));
                    }
                }
            };
        ((e[_e] = n), k(n), Lt());
    }
}
function ia(e, t) {
    var n = oa(e);
    n.value !== (n.value = t ?? void 0) &&
        (e.value !== t || (t === 0 && e.nodeName === na)) &&
        (e.value = t ?? ``);
}
function aa(e, t, n, r) {
    var i = oa(e);
    (w &&
        ((i[t] = e.getAttribute(t)),
        t === `src` || t === `srcset` || (t === `href` && e.nodeName === ta))) ||
        (i[t] !== (i[t] = n) &&
            (t === `loading` && (e[fe] = n),
            n == null
                ? e.removeAttribute(t)
                : typeof n != `string` && ca(e).includes(t)
                  ? (e[t] = n)
                  : e.setAttribute(t, n)));
}
function oa(e) {
    return (e[pe] ??= { [$i]: e.nodeName.includes(`-`), [ea]: e.namespaceURI === Be });
}
var sa = new Map();
function ca(e) {
    var t = e.getAttribute(`is`) || e.nodeName,
        n = sa.get(t);
    if (n) return n;
    sa.set(t, (n = []));
    for (var r, i = e, a = Element.prototype; a !== i;) {
        for (var o in ((r = c(i)), r))
            r[o].set && o !== `innerHTML` && o !== `textContent` && o !== `innerText` && n.push(o);
        i = d(i);
    }
    return n;
}
function la(e, t, n = t) {
    var r = new WeakSet();
    (zt(e, `input`, async (i) => {
        var a = i ? e.defaultValue : e.value;
        if (((a = da(e) ? fa(a) : a), n(a), j !== null && r.add(j), await Fr(), a !== (a = t()))) {
            var o = e.selectionStart,
                s = e.selectionEnd,
                c = e.value.length;
            if (((e.value = a ?? ``), s !== null)) {
                var l = e.value.length;
                o === s && s === c && l > c
                    ? ((e.selectionStart = l), (e.selectionEnd = l))
                    : ((e.selectionStart = o), (e.selectionEnd = Math.min(s, l)));
            }
        }
    }),
        ((w && e.defaultValue !== e.value) || (Z(t) == null && e.value)) &&
            (n(da(e) ? fa(e.value) : e.value), j !== null && r.add(j)),
        ar(() => {
            var n = t();
            if (e === document.activeElement) {
                var i = j;
                if (r.has(i)) return;
            }
            (da(e) && n === fa(e.value)) ||
                (e.type === `date` && !n && !e.value) ||
                (n !== e.value && (e.value = n ?? ``));
        }));
}
function ua(e, t, n = t) {
    (zt(e, `change`, (t) => {
        n(t ? e.defaultChecked : e.checked);
    }),
        ((w && e.defaultChecked !== e.checked) || Z(t) == null) && n(e.checked),
        ar(() => {
            e.checked = !!t();
        }));
}
function da(e) {
    var t = e.type;
    return t === `number` || t === `range`;
}
function fa(e) {
    return e === `` ? null : +e;
}
var pa = new (class e {
    #e = new WeakMap();
    #t;
    #n;
    static entries = new WeakMap();
    constructor(e) {
        this.#n = e;
    }
    observe(e, t) {
        var n = this.#e.get(e) || new Set();
        return (
            n.add(t),
            this.#e.set(e, n),
            this.#r().observe(e, this.#n),
            () => {
                var n = this.#e.get(e);
                (n.delete(t), n.size === 0 && (this.#e.delete(e), this.#t.unobserve(e)));
            }
        );
    }
    #r() {
        return (this.#t ??= new ResizeObserver((t) => {
            for (var n of t) {
                e.entries.set(n.target, n);
                for (var r of this.#e.get(n.target) || []) r(n);
            }
        }));
    }
})({ box: `border-box` });
function ma(e, t, n) {
    var r = pa.observe(e, () => n(e[t]));
    tr(() => (Z(() => n(e[t])), r));
}
function ha(e, t) {
    return e === t || e?.[ue] === t;
}
function ga(e = {}, t, n, r) {
    var i = O.r,
        a = W;
    return (
        tr(() => {
            var o, s;
            return (
                ar(() => {
                    ((o = s),
                        (s = r?.() || []),
                        Z(() => {
                            ha(n(...s), e) || (t(e, ...s), o && ha(n(...o), e) && t(null, ...o));
                        }));
                }),
                () => {
                    let r = a;
                    for (; r !== i && r.parent !== null && r.parent.f & 33554432;) r = r.parent;
                    let o = () => {
                            s && ha(n(...s), e) && t(null, ...s);
                        },
                        c = r.teardown;
                    r.teardown = () => {
                        (o(), c?.());
                    };
                }
            );
        }),
        e
    );
}
function _a(e = !1) {
    let t = O,
        n = t.l.u;
    if (!n) return;
    let r = () => zr(t.s);
    if (e) {
        let e = 0,
            n = {},
            i = Jt(() => {
                let r = !1,
                    i = t.s;
                for (let e in i) i[e] !== n[e] && ((n[e] = i[e]), (r = !0));
                return (r && e++, e);
            });
        r = () => X(i);
    }
    (n.b.length &&
        $n(() => {
            (va(t, r), g(n.b));
        }),
        Zn(() => {
            let e = Z(() => n.m.map(h));
            return () => {
                for (let t of e) typeof t == `function` && t();
            };
        }),
        n.a.length &&
            Zn(() => {
                (va(t, r), g(n.a));
            }));
}
function va(e, t) {
    if (e.l.s) for (let t of e.l.s) X(t);
    t();
}
var ya = {
    get(e, t) {
        let n = e.props.length;
        for (; n--;) {
            let r = e.props[n];
            if ((p(r) && (r = r()), typeof r == `object` && r && t in r)) return r[t];
        }
    },
    set(e, t, n) {
        let r = e.props.length;
        for (; r--;) {
            let i = e.props[r];
            p(i) && (i = i());
            let a = s(i, t);
            if (a && a.set) return (a.set(n), !0);
        }
        return !1;
    },
    getOwnPropertyDescriptor(e, t) {
        let n = e.props.length;
        for (; n--;) {
            let r = e.props[n];
            if ((p(r) && (r = r()), typeof r == `object` && r && t in r)) {
                let e = s(r, t);
                return (e && !e.configurable && (e.configurable = !0), e);
            }
        }
    },
    has(e, t) {
        if (t === ue || t === de) return !1;
        for (let n of e.props) if ((p(n) && (n = n()), n != null && t in n)) return !0;
        return !1;
    },
    ownKeys(e) {
        let t = [];
        for (let n of e.props)
            if ((p(n) && (n = n()), n)) {
                for (let e in n) t.includes(e) || t.push(e);
                for (let e of Object.getOwnPropertySymbols(n)) t.includes(e) || t.push(e);
            }
        return t;
    },
};
function ba(...e) {
    return new Proxy({ props: e }, ya);
}
function xa(e, t, n, r) {
    var i = !tt || !!(n & 2),
        a = !!(n & 8),
        o = !!(n & 16),
        c = r,
        l = !0,
        u = void 0,
        d = () => (o && i ? ((u ??= Jt(r)), X(u)) : (l && ((l = !1), (c = o ? Z(r) : r)), c));
    let f;
    if (a) {
        var p = ue in e || de in e;
        f = s(e, t)?.set ?? (p && t in e ? (n) => (e[t] = n) : void 0);
    }
    var m,
        h = !1;
    (a ? ([m, h] = Ft(() => e[t])) : (m = e[t]),
        m === void 0 && r !== void 0 && ((m = d()), f && (i && Pe(t), f(m))));
    var g = i
        ? () => {
              var n = e[t];
              return n === void 0 ? d() : ((l = !0), n);
          }
        : () => {
              var n = e[t];
              return (n !== void 0 && (c = void 0), n === void 0 ? c : n);
          };
    if (i && !(n & 4)) return g;
    if (f) {
        var _ = e.$$legacy;
        return function (e, t) {
            return arguments.length > 0 ? ((!i || !t || _ || h) && f(t ? g() : e), e) : g();
        };
    }
    var v = !1,
        y = (n & 1 ? Jt : Qt)(() => ((v = !1), g()));
    a && X(y);
    var b = W;
    return function (e, t) {
        if (arguments.length > 0) {
            let n = t ? X(y) : i && a ? Pn(e) : e;
            return (P(y, n), (v = !0), c !== void 0 && (c = n), e);
        }
        return (br && v) || b.f & 16384 ? y.v : X(y);
    };
}
function Sa(e) {
    return class extends Ca {
        constructor(t) {
            super({ component: e, ...t });
        }
    };
}
var Ca = class {
    #e;
    #t;
    constructor(e) {
        var t = new Map(),
            n = (e, n) => {
                var r = kn(n, !1, !1);
                return (t.set(e, r), r);
            };
        let r = new Proxy(
            { ...(e.props || {}), $$events: {} },
            {
                get(e, r) {
                    return X(t.get(r) ?? n(r, Reflect.get(e, r)));
                },
                has(e, r) {
                    return r === de || (X(t.get(r) ?? n(r, Reflect.get(e, r))), Reflect.has(e, r));
                },
                set(e, r, i) {
                    return (P(t.get(r) ?? n(r, i), i), Reflect.set(e, r, i));
                },
            }
        );
        ((this.#t = (e.hydrate ? ui : li)(e.component, {
            target: e.target,
            anchor: e.anchor,
            props: r,
            context: e.context,
            intro: e.intro ?? !1,
            recover: e.recover,
            transformError: e.transformError,
        })),
            (!e?.props?.$$host || e.sync === !1) && hn(),
            (this.#e = r.$$events));
        for (let e of Object.keys(this.#t))
            e !== `$set` &&
                e !== `$destroy` &&
                e !== `$on` &&
                o(this, e, {
                    get() {
                        return this.#t[e];
                    },
                    set(t) {
                        this.#t[e] = t;
                    },
                    enumerable: !0,
                });
        ((this.#t.$set = (e) => {
            Object.assign(r, e);
        }),
            (this.#t.$destroy = () => {
                mi(this.#t);
            }));
    }
    $set(e) {
        this.#t.$set(e);
    }
    $on(e, t) {
        this.#e[e] = this.#e[e] || [];
        let n = (...e) => t.call(this, ...e);
        return (
            this.#e[e].push(n),
            () => {
                this.#e[e] = this.#e[e].filter((e) => e !== n);
            }
        );
    }
    $destroy() {
        this.#t.$destroy();
    }
};
function wa(e, t) {
    if ((be(`hydratable`), w)) {
        let t = window.__svelte?.h;
        if (t?.has(e)) return t.get(e);
        We(e);
    }
    return t();
}
var Ta = t({
    afterUpdate: () => Ma,
    beforeUpdate: () => ja,
    createContext: () => it,
    createEventDispatcher: () => Aa,
    createRawSnippet: () => ki,
    flushSync: () => hn,
    fork: () => Sn,
    getAbortSignal: () => Ea,
    getAllContexts: () => ct,
    getContext: () => at,
    hasContext: () => st,
    hydratable: () => wa,
    hydrate: () => ui,
    mount: () => li,
    onDestroy: () => Oa,
    onMount: () => Da,
    setContext: () => ot,
    settled: () => Ir,
    tick: () => Fr,
    unmount: () => mi,
    untrack: () => Z,
});
function Ea() {
    return (V === null && je(), (V.ac ??= new AbortController()).signal);
}
function Da(e) {
    (O === null && xe(`onMount`),
        tt && O.l !== null
            ? Na(O).m.push(e)
            : Zn(() => {
                  let t = Z(e);
                  if (typeof t == `function`) return t;
              }));
}
function Oa(e) {
    (O === null && xe(`onDestroy`), Da(() => () => Z(e)));
}
function ka(e, t, { bubbles: n = !1, cancelable: r = !1 } = {}) {
    return new CustomEvent(e, { detail: t, bubbles: n, cancelable: r });
}
function Aa() {
    let e = O;
    return (
        e === null && xe(`createEventDispatcher`),
        (t, r, i) => {
            let a = e.s.$$events?.[t];
            if (a) {
                let o = n(a) ? a.slice() : [a],
                    s = ka(t, r, i);
                for (let t of o) t.call(e.x, s);
                return !s.defaultPrevented;
            }
            return !0;
        }
    );
}
function ja(e) {
    (O === null && xe(`beforeUpdate`), O.l === null && Ne(`beforeUpdate`), Na(O).b.push(e));
}
function Ma(e) {
    (O === null && xe(`afterUpdate`), O.l === null && Ne(`afterUpdate`), Na(O).a.push(e));
}
function Na(e) {
    var t = e.l;
    return (t.u ??= { a: [], b: [], m: [] });
}
export {
    P as $,
    ai as A,
    Fr as B,
    Oi as C,
    gi as D,
    _i as E,
    Jr as F,
    Zn as G,
    nr as H,
    qr as I,
    Bn as J,
    $n as K,
    zr as L,
    ri as M,
    ii as N,
    ci as O,
    Yr as P,
    kn as Q,
    X as R,
    Ai as S,
    xi as T,
    rr as U,
    Z as V,
    or as W,
    Hn as X,
    Vn as Y,
    Pn as Z,
    Wi as _,
    xa as a,
    jt as at,
    Ni as b,
    ga as c,
    ut as ct,
    la as d,
    nt as dt,
    On as et,
    ra as f,
    Ye as ft,
    Xi as g,
    Qi as h,
    Sa as i,
    Pt as it,
    ti as j,
    oi as k,
    ma as l,
    lt,
    ia as m,
    t as mt,
    Oa as n,
    Mt as nt,
    ba as o,
    Tt as ot,
    aa as p,
    Je as pt,
    Fn as q,
    Da as r,
    At as rt,
    _a as s,
    at as st,
    Ta as t,
    Zt as tt,
    ua as u,
    ot as ut,
    zi as v,
    Di as w,
    $ as x,
    Bi as y,
    Ir as z,
};
